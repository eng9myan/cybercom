...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(
            entry_strategy=get_param(
                "odoo_biometric_attendance.entry_strategy", default="1"
            ),
            update_device=get_param(
                "odoo_biometric_attendance.update_device", default=False
            ),
            device_api_base_url=get_param(
                "odoo_biometric_attendance.device_api_base_url",
                default="http://robot.camsunit.com/external/1.0/user",
            ),
        )
        return res
    ...
    def set_values(self):
        ...
        set_param("odoo_biometric_attendance.entry_strategy", self.entry_strategy)
        set_param("odoo_biometric_attendance.update_device", self.update_device)
        set_param(
            "odoo_biometric_attendance.device_api_base_url", self.device_api_base_url
        )
