...
export default function SalesApprovalsFlow() {
  ...
  const handleAction = (id: string) => setHiddenIds((prev) => [...prev, id]);
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1 className="page-title text-white">{t('salesApprovals.title')}</h1>
          <p className="page-subtitle">{t('salesApprovals.subtitle')}</p>
        </div>
      </div>

      <div className="glass-card p-6 border-cyan-500/20 bg-cyan-950/10 text-xs">
        <h3 className="text-sm font-bold text-white mb-2">{t('salesApprovals.workflowsHeading')}</h3>
        <p className="text-slate-400 leading-relaxed mb-4">{t('salesApprovals.workflowsBody')}</p>
      </div>

      <div className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-400">{t('salesApprovals.queue')}</h2>
        {list.length === 0 ? (
          <div className="glass-card p-8 text-center text-slate-500 text-xs font-semibold">
            {t('salesApprovals.cleanQueue')}
          </div>
        ) : (
          list.map((item) => (
            <div key={item.id} className="glass-card p-5 space-y-4">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs text-slate-400 block mt-1.5">{t('salesApprovals.value')}: <strong className="text-white">{item.totalAmount}</strong></span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 text-xs font-mono bg-black/30 p-3 rounded-lg border border-white/5 text-slate-300">
                <div>
                  <span className="text-slate-500 block">{t('salesApprovals.requestedDiscount')}</span>
                  <span className="text-rose-400 font-bold text-sm">{item.requestedDiscount}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">{t('salesApprovals.allowedLimit')}</span>
                  <span className="text-slate-300 font-bold text-sm">{item.limitDiscount}</span>
                </div>
                <div className="col-span-2 pt-2 border-t border-white/5">
                  <span className="text-slate-500 block font-sans">{t('salesApprovals.repReasonNote')}</span>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-white/5">
                ...
                >
                  <X className="w-3.5 h-3.5" /> {t('salesApprovals.denyOverride')}
                </button>
                <button
                  onClick={() => handleAction(item.id)}
                ...
                >
                  <Check className="w-3.5 h-3.5" /> {t('salesApprovals.approveOverride')}
                </button>
              </div>
            </div>
          ))
