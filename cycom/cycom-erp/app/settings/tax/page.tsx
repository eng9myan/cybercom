    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8 text-xs md:text-sm">
      <div className="max-w-4xl mx-auto flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">{t('settingsTax.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 space-y-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2 border-b border-white/5 pb-2">
            <Percent className="w-4 h-4 text-blue-400" /> {t('settingsTax.configHeading')}
          </h3>
          <form onSubmit={handleCalculate} className="space-y-4">
            <div className="space-y-1">
              <label className="text-slate-400">{t('settingsTax.vatRate')}</label>
              <input
            ...
            <div className="space-y-1">
              <label className="text-slate-400">{t('settingsTax.whtRate')}</label>
              <input
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              />
              <label htmlFor="wht" className="text-slate-300">{t('settingsTax.applyWht')}</label>
            </div>

            <div className="space-y-1 border-t border-white/5 pt-3">
              <label className="text-slate-400 font-bold block mb-1">{t('settingsTax.testValue')}</label>
              <input
          ...
          {calcResult ? (
            <>
              ...
              <div className="glass-card p-6 space-y-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2 border-b border-white/5 pb-2">
                  {t('settingsTax.breakdownHeading')}
                </h3>
                <div className="space-y-1.5 text-xs font-medium">
                  <div className="flex justify-between"><span className="text-slate-500">{t('settingsTax.baseAmount')}</span><span className="text-slate-200">{calcResult.base_amount.toFixed(2)} JOD</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">{t('settingsTax.vatLine', { rate: calcResult.vat_rate * 100 })}</span><span className="text-slate-200">+{calcResult.vat_amount.toFixed(2)} JOD</span></div>
