...
export default function AIReconciliation() {
  ...
  const handleRunReconciliation = async () => {
    ...
    try {
      ...
      alert(t('accountingReconciliation.matchFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8 text-xs md:text-sm">
      ...
      <div className="max-w-5xl mx-auto flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent flex items-center gap-2">
              <Sparkles className="w-6 h-6 text-indigo-400" /> {t('accountingReconciliation.title')}
            </h1>
        ...
        >
          <Brain className="w-4 h-4" /> {t('accountingReconciliation.runEngine')}
        </button>
      </div>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="glass-card p-6">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 border-b border-white/5 pb-2">
            {t('accountingReconciliation.bankEntriesHeading')}
          </h3>
            ...
            {bankLines.map(l => (
              <div key={l.id} className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl flex justify-between items-center">
                <div>
                  ...
                  <div className="text-[10px] text-slate-500 mt-0.5">{t('accountingReconciliation.lineId', { id: l.id })}</div>
                </div>
        ...
        <div className="glass-card p-6">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 border-b border-white/5 pb-2">
            {t('accountingReconciliation.invoicesHeading')}
          </h3>
          <div className="space-y-2">
            {invoices.map(i => (
      ...
      {results.length > 0 && (
        <div className="max-w-5xl mx-auto glass-card p-6">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-4 border-b border-white/5 pb-2">
            {t('accountingReconciliation.resultsHeading')}
          </h3>
          <div className="overflow-x-auto">
            <table className="data-table w-full text-start">
              <thead>
                <tr className="border-b border-white/5 text-[10px] text-slate-500 uppercase tracking-wider font-semibold">
                  <th className="pb-3">{t('accountingReconciliation.colBankLineId')}</th>
                  <th className="pb-3">{t('accountingReconciliation.colMatchedInvoice')}</th>
                  <th className="pb-3">{t('accountingReconciliation.colPartnerName')}</th>
                  <th className="pb-3">{t('accountingReconciliation.colAmount')}</th>
                  <th className="pb-3">{t('accountingReconciliation.colConfidence')}</th>
                  <th className="pb-3">{t('accountingReconciliation.colMatchingStatus')}</th>
                </tr>
              </thead>
