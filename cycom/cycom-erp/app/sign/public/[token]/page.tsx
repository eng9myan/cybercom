...
export default function PublicSignPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/api/sign/requests/public/${token}`)
      .then(res => {
        if (!res.ok) throw new Error(t('publicSignPage.verificationFailed'));
        return res.json();
      })
      .then(resData => {
        setData(resData);
    ...
    return (
      <div className="min-h-screen bg-[#030712] text-white flex items-center justify-center font-sans">
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-400 text-xs font-bold">{t('publicSignPage.verifying')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#030712] text-white flex items-center justify-center p-6 font-sans">
        <div className="w-full max-w-md bg-[#0B0F19] border border-white/5 p-8 rounded-2xl text-center space-y-4 shadow-2xl">
          ...
          <h2 className="text-lg font-black text-white">{t('publicSignPage.verificationFailed')}</h2>
  ...
  const signer = data.request.signers?.[0] || { name: t('publicSignPage.recipient'), email: '' };
      ...
      <div className="max-w-4xl mx-auto space-y-6 relative z-10">
        ...
        <header className="flex items-center justify-between border-b border-white/5 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-baseline gap-0.5">
                ...
                <span className="text-[8px] font-bold px-1 rounded bg-rose-500/20 text-[#FB7185] ms-1.5 border border-rose-500/30">SIGN</span>
              </div>
              <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{t('publicSignPage.portalSubtitle')}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] font-bold">
            <Shield className="w-3.5 h-3.5" /> {t('publicSignPage.securedBadge')}
          </div>
        </header>
        ...
        {success ? (
          ...
          >
            ...
            <div className="space-y-1.5">
              <h2 className="text-lg font-black text-white">{t('publicSignPage.signedTitle')}</h2>
              <p className="text-xs text-slate-400">{t('publicSignPage.signedSubtitle')}</p>
            </div>

            <div className="p-4 bg-white/2 border border-white/5 rounded-xl text-start space-y-2 font-mono text-[9px] text-slate-500">
              <div><span className="font-bold text-slate-400">{t('publicSignPage.contractLabel')}</span> {data.template.name}</div>
              <div><span className="font-bold text-slate-400">{t('publicSignPage.signerLabel')}</span> {signer.name} ({signer.email})</div>
              <div><span className="font-bold text-slate-400">{t('publicSignPage.fingerprintLabel')}</span> <span dir="ltr">sha256-{token.substring(0, 16)}...</span></div>
              <div><span className="font-bold text-slate-400">{t('publicSignPage.timestampLabel')}</span> <span dir="ltr">{new Date().toUTCString()}</span></div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="glass-card p-5 space-y-5">
              <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 border-b border-white/5 pb-2">{t('publicSignPage.instructionsHeading')}</h3>
              ...
              <div className="space-y-4 text-xs">
                <div className="flex gap-3">
                  ...
                  <p className="text-slate-400 leading-relaxed">{t('publicSignPage.step1')}</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-slate-400 leading-relaxed">{t('publicSignPage.step2Pre')} <strong>{t('publicSignPage.step2Strong')}</strong>{t('publicSignPage.step2Post')}</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-slate-400 leading-relaxed">{t('publicSignPage.step3Pre')} <strong>{t('publicSignPage.step3Strong')}</strong>{t('publicSignPage.step3Post')}</p>
                </div>
              </div>

              <div className="border-t border-white/5 pt-4 space-y-3 text-xs">
                <div className="flex justify-between items-center text-[10px] text-slate-500 uppercase font-bold">{t('publicSignPage.docDetailsHeading')}</div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="glass-card p-6 md:p-8 bg-white/[0.01] border-white/5 min-h-[600px] flex flex-col justify-between relative">
                ...
                <div className="space-y-6">
                  <div className="text-center space-y-2 border-b border-white/5 pb-6">
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{t('publicSignPage.docDetailsHeading')}</p>
                  </div>

                  <div className="text-slate-300 text-xs leading-relaxed space-y-4 font-light">
                    <p>{t('publicSignPage.agreementIntro', { date: signedDate, signer: signer.name })}</p>
                    <p className="text-slate-400 uppercase text-[9px] font-bold tracking-wider">{t('publicSignPage.clause1Heading')}</p>
                    <p>{t('publicSignPage.clause1Body')}</p>
                    <p className="text-slate-400 uppercase text-[9px] font-bold tracking-wider">{t('publicSignPage.clause2Heading')}</p>
                    <p>{t('publicSignPage.clause2Body')}</p>
                    <p className="text-slate-400 italic">{t('publicSignPage.clauseFooter')}</p>
                  </div>
                </div>
                        ...
                        >
                          {signatureImage ? (
                            ...
                            <>
                              ...
                              <span className="text-[10px] font-bold text-rose-400 uppercase tracking-wider">{t('publicSignPage.clickToSign')}</span>
                            </>
                          )}
                          <span className="absolute bottom-1.5 start-2 text-[8px] text-slate-500 uppercase font-bold font-mono">{t('publicSignPage.signerSignatureBlock')}</span>
                        </div>
                      );
                    }
                    ...
                    if (field.type === 'text') {
                      return (
                        <div key={idx} className="border border-white/5 bg-white/2 p-4 rounded-xl min-h-[100px] flex flex-col justify-end relative">
                          ...
                          <span className="absolute bottom-1.5 start-2 text-[8px] text-slate-500 uppercase font-bold font-mono">{t('publicSignPage.fullNameBlock')}</span>
                        </div>
                      );
                    }
                    ...
                    if (field.type === 'date') {
                      return (
                        <div key={idx} className="border border-white/5 bg-white/2 p-4 rounded-xl min-h-[100px] flex flex-col justify-end relative">
                          ...
                          <span className="absolute bottom-1.5 start-2 text-[8px] text-slate-500 uppercase font-bold font-mono">{t('publicSignPage.dateBlock')}</span>
                        </div>
                      );
                    }
                    return null;
              ...
              >
                {signing ? (
                  ...
                  <>
                    <CheckCircle className="w-5 h-5" /> {t('publicSignPage.finalizeButton')}
                  </>
                )}
              </button>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            ...
            >
              <div className="flex justify-between items-center border-b border-white/5 pb-3">
                <h3 className="text-base font-black text-white">{t('publicSignPage.modalTitle')}</h3>
              ...
              {signMethod === 'draw' ? (
                <div className="space-y-3">
                  <div className="bg-black/60 rounded-xl border border-white/10 overflow-hidden relative touch-none">
                    ...
                    />
                    <div className="absolute top-2 start-2 text-[8px] text-slate-500 font-bold uppercase tracking-wider pointer-events-none">
                      {t('publicSignPage.drawHint')}
                    </div>
                  </div>
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <Trash2 className="w-3.5 h-3.5" /> {t('publicSignPage.clearPad')}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4 text-xs">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">{t('publicSignPage.fullNameLabel')}</label>
                    <input
                      type="text"
                      value={typedName}
                      onChange={e => setTypedName(e.target.value)}
                      placeholder={t('publicSignPage.fullNamePh')}
                      className="input-field py-2.5"
                    />
                  </div>

                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-slate-500 uppercase">{t('publicSignPage.styleLabel')}</span>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { id: 'font-sans italic', label: t('publicSignPage.styleModern') },
                        { id: 'font-serif italic', label: t('publicSignPage.styleClassic') },
                        { id: 'font-cursive', label: t('publicSignPage.styleCursive') }
                      ].map((style) => (
                        <button
                          key={style.id}
                        ...
                        >
                          <span className={`text-sm truncate block ${style.id}`}>{typedName || t('publicSignPage.signaturePlaceholder')}</span>
              ...
              >
                {t('publicSignPage.insertSignature')}
              </button>
            </motion.div>
          </div>
        )}
