...
import { Upload, FileText, Search, X, Check, Copy, ExternalLink, Send } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white">{t('signTemplates.title')}</h2>
          <p className="text-xs text-slate-400">{t('signTemplates.subtitle')}</p>
        </div>
        <div className="flex gap-3">
          <div className="relative w-64">
            ...
            <input
              type="text"
              placeholder={t('signTemplates.searchPh')}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-xl ps-9 pe-4 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/50 transition-all"
            />
          </div>
          <button
      ...
      <div className="glass-card p-6 min-h-[400px]">
        {loading ? (
          <div className="flex justify-center items-center h-[300px] text-slate-400">{t('signTemplates.loading')}</div>
        ) : filteredTemplates.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 border border-white/10">
              <FileText className="w-8 h-8 text-slate-500" />
            </div>
            <h3 className="text-white font-bold mb-2">{t('signTemplates.emptyTitle')}</h3>
            <p className="text-sm text-slate-400 max-w-sm mb-6">{t('signTemplates.emptyBody')}</p>
          </div>
        ) : (
            ...
            {filteredTemplates.map((tpl: any) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 font-mono mt-1">{t('signTemplates.uploadedOn', { date: new Date(tpl.created_at).toLocaleDateString() })}</p>
            ...
            >
              <div className="flex justify-between items-center border-b border-white/5 pb-3">
                <h3 className="text-base font-black text-white flex items-center gap-2">
                  <Upload className="w-5 h-5 text-rose-400" /> {t('signTemplates.modalUploadTitle')}
                </h3>
              ...
              <form onSubmit={handleUploadSubmit} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">{t('signTemplates.templateName')}</label>
                      <input
                        type="text"
                        required
                        placeholder={t('signTemplates.templateNamePh')}
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">{t('signTemplates.pdfFile')}</label>
                      <div className="border border-dashed border-white/10 rounded-xl p-6 text-center hover:border-rose-500/50 transition-colors relative cursor-pointer bg-white/2">
                        ...
                        <p className="text-white font-bold text-[11px] truncate">
                          {selectedFile ? selectedFile.name : t('signTemplates.selectOrDrag')}
                        </p>
                        <p className="text-[10px] text-slate-500 mt-1">{t('signTemplates.pdfOnlyNote')}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 bg-white/3 border border-white/5 p-4 rounded-xl flex flex-col">
                    <span className="text-[10px] font-bold text-slate-500 uppercase">{t('signTemplates.mapFieldsHeading')}</span>
                    <p className="text-[10px] text-slate-400 leading-relaxed">{t('signTemplates.mapFieldsNote')}</p>
                    ...
                    <div className="space-y-2 mt-2">
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg bg-white/2 hover:bg-white/5 border border-white/5">
                        ...
                        />
                        <span className="text-white font-semibold">{t('signTemplates.placeSignature')}</span>
                      </label>
                      ...
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg bg-white/2 hover:bg-white/5 border border-white/5">
                        <input
                          ...
                          className="rounded bg-white/5 border-white/10 text-rose-500"
                        />
                        <span className="text-white font-semibold">{t('signTemplates.placeName')}</span>
                      </label>
                      ...
                      <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg bg-white/2 hover:bg-white/5 border border-white/5">
                        <input
                          ...
                          className="rounded bg-white/5 border-white/10 text-rose-500"
                        />
                        <span className="text-white font-semibold">{t('signTemplates.placeDate')}</span>
                      </label>
                    </div>
                    ...
                    <div className="mt-4 flex-1 h-[140px] rounded border border-white/5 bg-white/2 flex flex-col justify-end p-4 relative overflow-hidden">
                      <div className="absolute top-2 start-2 end-2 border-b border-white/5 pb-1">
                      ...
                      <div className="space-y-2.5">
                        {reqSignature && (
                          <div className="border border-rose-500/20 bg-rose-500/5 p-1 rounded text-[8px] text-rose-400 font-bold text-center border-dashed">
                            {t('signTemplates.signaturePlaceMock')}
                          </div>
                        )}
                        <div className="grid grid-cols-2 gap-2">
                          {reqName && (
                            <div className="border border-white/10 bg-white/5 p-1 rounded text-[7px] text-slate-400 font-semibold text-center truncate">
                              {t('signTemplates.fullNameBlockMock')}
                            </div>
                          )}
                          {reqDate && (
                            <div className="border border-white/10 bg-white/5 p-1 rounded text-[7px] text-slate-400 font-semibold text-center truncate">
                              {t('signTemplates.dateBlockMock')}
                            </div>
                          )}
                        </div>
                      </div>
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      <Check className="w-4 h-4" /> {t('signTemplates.saveTemplate')}
                    </>
                  )}
                </button>
              </form>
            ...
            >
              <div className="flex justify-between items-center border-b border-white/5 pb-3">
                <h3 className="text-base font-black text-white flex items-center gap-2 truncate">
                  <Send className="w-5 h-5 text-rose-400" /> {t('signTemplates.modalSendTitle', { name: selectedTemplate.name })}
                </h3>
              ...
              {!generatedRequest ? (
                <form onSubmit={handleRequestSubmit} className="space-y-4 text-xs">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">{t('signTemplates.signerName')}</label>
                    <input
                      type="text"
                      required
                      placeholder={t('signTemplates.signerNamePh')}
                      value={recipientName}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">{t('signTemplates.signerEmail')}</label>
                    <input
                      type="email"
                      required
                      placeholder={t('signTemplates.signerEmailPh')}
                  ...
                  >
                    {sendingRequest ? (
                      ...
                      <>
                        <Send className="w-4 h-4" /> {t('signTemplates.sendRequestBtn')}
                      </>
                    )}
                  </button>
                </form>
                ...
                <div className="space-y-4 text-center py-2 text-xs">
                  ...
                  <div>
                    <h4 className="text-white font-bold">{t('signTemplates.requestSent')}</h4>
                    <p className="text-[10px] text-slate-500 mt-1">{t('signTemplates.requestSentNote')}</p>
                  </div>

                  <div className="p-3 bg-white/3 border border-white/5 rounded-xl flex items-center justify-between gap-3 text-start font-mono text-[10px] text-slate-400">
                    ...
                    >
                      {copied ? t('signTemplates.copied') : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    ...
                    >
                      <span>{t('signTemplates.openPortal')}</span>
