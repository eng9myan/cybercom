  ...
  return (
    <div className="space-y-6">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title text-white">{t('signDash.title')}</h1>
          <p className="page-subtitle">{t('signDash.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="stat-card flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('signDash.awaiting')}</span>
        ...
        <div className="stat-card flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('signDash.completed')}</span>
        ...
        <div className="stat-card flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('signDash.sentReq')}</span>
        ...
        <div className="stat-card flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('signDash.activeTemplates')}</span>
      ...
      <div className="glass-card p-6">
        <h3 className="text-sm font-bold text-white mb-4">{t('signDash.recentHeading')}</h3>
        {loading ? (
          <p className="text-slate-500 text-xs italic">{t('signDash.loadingActivities')}</p>
        ) : recentRequests.length === 0 ? (
          <div className="text-center py-10 text-slate-500 text-xs italic">
            {t('signDash.emptyStatePrefix')}{' '}
            <Link href="/sign/templates" className="text-rose-400 hover:underline">{t('signDash.templatesLink')}</Link>
            {' '}{t('signDash.emptyStateSuffix')}
          </div>
        ) : (
              ...
              return (
                <div key={req.id} className="p-4 rounded-xl bg-white/3 border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-300 font-bold">{t('signDash.recipient', { name: signer.name, email: signer.email ? `(${signer.email})` : '' })}</p>
                  </div>
                  ...
                  {req.status !== 'Signed' && (
                    ...
                    >
                      <span>{t('signDash.accessPortal')}</span>
