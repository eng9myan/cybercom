...
describe("t()", () => {
  it("resolves a dotted key", () => {
    expect(t("common.retry")).toBe("Retry"); // jsdom localStorage empty -> en
    expect(t("overview.title")).toBe("Provider Overview");
  });

  it("returns the key unchanged when missing", () => {
    expect(t("overview.nope.missing")).toBe("overview.nope.missing");
  });
