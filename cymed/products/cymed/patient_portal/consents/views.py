...
class PortalConsentRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return PortalConsentRecord.objects.filter(tenant_id=self.request.tenant_id).select_related(
            "consent_type"
        )
    ...
    def grant(self, request, pk=None):
        ...
        record.save(
            update_fields=["consent_status", "granted_at", "version_consented", "updated_at"]
        )
...
class ConsentRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return ConsentRequest.objects.filter(tenant_id=self.request.tenant_id).select_related(
            "consent_type"
        )
...
class ConsentHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return ConsentHistory.objects.filter(tenant_id=self.request.tenant_id).select_related(
            "consent_record"
        )
