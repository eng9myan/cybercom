...
class TelemedicineDocument(BaseModel):
    ...
    uploaded_by = models.CharField(max_length=20, choices=UPLOADED_BY_CHOICES, default="patient")
...
class TelemedicineChat(BaseModel):
    ...
    sender_type = models.CharField(max_length=20, choices=SENDER_TYPE_CHOICES, default="patient")
...
class TelemedicineRating(BaseModel):
    ...
    def __str__(self):
        return (
            f"TelemedicineRating {self.id} for session {self.session_id} - {self.overall_rating}/5"
        )
