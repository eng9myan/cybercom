...
used ONLY by core.settings_local.

It performs NO signature verification. It reads claims from a Bearer token if
one is present (so a frontend's unsigned dev JWT flows through), otherwise it
...
Hard safety gate: refuses to activate unless DEBUG and CYMED_DEV_AUTH=1.
"""
...
class DevAuthMiddleware:
    def __init__(self, get_response):
        if not (settings.DEBUG and os.environ.get("CYMED_DEV_AUTH") == "1"):
            raise RuntimeError(
                "DevAuthMiddleware refuses to load without DEBUG=True and CYMED_DEV_AUTH=1. "
    ...
    def __call__(self, request):
        ...
        request.user_session = {
            ...
            "email": claims.get("email", "physician@cymed.dev"),
            "tenant_id": claims.get("tenant_id", self.dev_tenant),
            "roles": claims.get("roles")
            or claims.get("realm_access", {}).get("roles", ["physician"]),
