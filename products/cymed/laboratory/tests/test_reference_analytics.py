...
def make_order_item(tenant_id):
    ...
    test = LabTest.objects.create(tenant_id=tenant_id, code=f"REF-{uuid.uuid4().hex[:4]}", name="Reference Test")
...
class TestReferenceLab:
    def test_create_reference_lab(self):
        ...
        lab = ReferenceLab.objects.create(
            tenant_id=TENANT, code="REFLAB-01",
    ...
    def test_reference_lab_routing(self):
        ...
        lab = ReferenceLab.objects.create(
            tenant_id=TENANT, code=f"REFLAB-{uuid.uuid4().hex[:4]}",
            name="Ref Lab", integration_type="fhir",
        )
        test = LabTest.objects.create(tenant_id=TENANT, code=f"MOLTEST-{uuid.uuid4().hex[:4]}", name="Molecular Test")
        routing = ReferenceLabRouting.objects.create(
            tenant_id=TENANT, test=test, reference_lab=lab,
            is_default=True, estimated_tat_hours=72,
        )
    ...
    def test_reference_lab_order(self):
        ...
        lab = ReferenceLab.objects.create(
            tenant_id=TENANT, code=f"REFLAB-{uuid.uuid4().hex[:4]}",
            name="Ref Lab", integration_type="api_rest",
        )
        item = make_order_item(TENANT)
        ref_order = ReferenceLabOrder.objects.create(
            tenant_id=TENANT, local_order_item=item, reference_lab=lab,
            external_order_id="EXT-00123",
        )
        assert ref_order.status == "pending"

    def test_reference_lab_result(self):
        from products.cymed.laboratory.reference_lab.models import ReferenceLab, ReferenceLabOrder, ReferenceLabResult
        lab = ReferenceLab.objects.create(
            tenant_id=TENANT, code=f"REFLAB-{uuid.uuid4().hex[:4]}",
            name="Ref Lab", integration_type="fhir",
        )
        ...
        result = ReferenceLabResult.objects.create(
            tenant_id=TENANT, reference_lab_order=ref_order,
            raw_result={"resourceType": "DiagnosticReport"}, raw_format="fhir",
        )
...
class TestBloodBank:
    def test_blood_product(self):
        ...
        unit = BloodProduct.objects.create(
            tenant_id=TENANT, unit_number="UNIT-001",
            product_type="prbc", blood_group="O", rh_type="negative",
    ...
    def test_blood_inventory(self):
        ...
        inv = BloodInventory.objects.create(
            tenant_id=TENANT, product_type="prbc",
            blood_group="O", rh_type="negative",
            available_units=10, minimum_threshold=2,
        )
    ...
    def test_blood_compatibility(self):
        ...
        compat = BloodCompatibility.objects.create(
            ...
            blood_group="A", rh_type="positive",
    ...
    def test_transfusion_request(self):
        ...
        req = TransfusionRequest.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            product_type="prbc", units_requested=2,
            urgency="urgent", requested_by=PROVIDER,
        )
...
class TestAnalytics:
    def test_ops_dashboard_snapshot(self):
        ...
        snap = LabOperationsDashboard.objects.create(
            tenant_id=TENANT, snapshot_date=datetime.date.today(),
            department="chemistry",
            total_orders=150, orders_completed=145,
            critical_results=3, qc_failures=1,
    ...
    def test_tat_metric(self):
        ...
        metric = LabTurnaroundMetric.objects.create(
            tenant_id=TENANT, department="hematology",
            test_code="CBC", test_name="CBC",
            period_type="daily",
    ...
    def test_quality_dashboard(self):
        ...
        dash = LabQualityDashboard.objects.create(
            tenant_id=TENANT, period_month=6, period_year=2026,
            department="chemistry",
...
class TestFHIRMapping:
    def test_fhir_service_request_mapping(self):
        ...
        order = LabOrder.objects.create(
            ...
            order_number="LAB-FHIR-001", priority="stat",
        )
    ...
    def test_fhir_specimen_mapping(self):
        ...
        sp = Specimen.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            specimen_number="SP-FHIR-001", barcode="BC-FHIR-001",
            specimen_type="blood",
        )
    ...
    def test_fhir_observation_mapping(self):
        ...
        test = LabTest.objects.create(tenant_id=TENANT, code=f"TSH-{uuid.uuid4().hex[:4]}", name="TSH")
        order = LabOrder.objects.create(
            tenant_id=TENANT, patient_id=PATIENT, ordered_by=PROVIDER,
            order_number=f"ORD-{uuid.uuid4().hex[:8]}",
        )
        ...
        rv = ResultValue.objects.create(
            ...
            analyte_code="TSH", analyte_name="Thyroid Stimulating Hormone",
            loinc_code="3016-3", value_type="numeric",
            value_numeric=Decimal("2.5"), unit="mIU/L", interpretation="N",
        )
...
class TestEditionFeatureMap:
    ...
    def test_lab_advanced_is_superset_of_basic(self):
        from products.cymed.commercial.feature_flags.services import LAB_BASIC_FEATURES, LAB_ADVANCED_FEATURES
    ...
    def test_lab_reference_is_superset_of_advanced(self):
        from products.cymed.commercial.feature_flags.services import LAB_ADVANCED_FEATURES, LAB_REFERENCE_FEATURES
    ...
    def test_lab_ordering_service_validates_create(self):
        ...
        result = LabOrderingService.create_order(
            tenant_id=TENANT, patient_id=PATIENT,
            order_type="clinic", tests=["GLUCX"], priority="routine",
            ordered_by=PROVIDER,
        )
