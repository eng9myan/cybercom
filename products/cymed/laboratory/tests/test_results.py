...
def make_order_item(tenant_id):
    ...
    test = LabTest.objects.create(tenant_id=tenant_id, code=f"TEST-{uuid.uuid4().hex[:4]}", name="Test")
...
class TestLabResult:
    ...
    def test_result_value_numeric(self):
        ...
        rv = ResultValue.objects.create(
            ...
            analyte_code="HGB", analyte_name="Hemoglobin",
            loinc_code="718-7", value_type="numeric",
            value_numeric=Decimal("14.5"), unit="g/dL",
            interpretation="N",
        )
    ...
    def test_critical_result_creation(self):
        from products.cymed.laboratory.results.models import LabResult, ResultValue, CriticalResult
        item = make_order_item(TENANT)
        result = LabResult.objects.create(tenant_id=TENANT, order_item=item)
        rv = ResultValue.objects.create(
            tenant_id=TENANT, result=result,
            analyte_code="K", analyte_name="Potassium",
            value_type="numeric", value_numeric=Decimal("6.8"),
            unit="mEq/L", interpretation="HH", is_critical=True,
        )
    ...
    def test_reference_range(self):
        ...
        test = LabTest.objects.create(tenant_id=TENANT, code=f"HGB-{uuid.uuid4().hex[:4]}", name="Hemoglobin")
        rr = ReferenceRange.objects.create(
            tenant_id=TENANT, test=test, sex="M",
            age_min_years=Decimal("18"), age_max_years=Decimal("65"),
            value_low=Decimal("13.5"), value_high=Decimal("17.5"),
            critical_low=Decimal("7.0"), critical_high=Decimal("20.0"),
            unit="g/dL",
        )
    ...
    def test_result_approval(self):
        ...
        approval = ResultApproval.objects.create(
            tenant_id=TENANT, result=result,
            approval_level="pathologist", approved_by=PROVIDER,
        )
    ...
    def test_result_correction(self):
        ...
        corr = ResultCorrection.objects.create(
            tenant_id=TENANT, result=result,
            analyte_code="NA", original_value="125",
            corrected_value="135", correction_reason="Transcription error",
...
class TestQualityControl:
    def test_create_qc_material(self):
        ...
        test = LabTest.objects.create(tenant_id=TENANT, code=f"QC-TST-{uuid.uuid4().hex[:4]}", name="QC Test")
        import datetime
        qc = QualityControl.objects.create(
            tenant_id=TENANT, test=test,
    ...
    def test_qc_run_pass(self):
        ...
        test = LabTest.objects.create(tenant_id=TENANT, code=f"QC-TST-{uuid.uuid4().hex[:4]}", name="QC Test")
        qc = QualityControl.objects.create(
            tenant_id=TENANT, test=test, lot_number=f"LOT-{uuid.uuid4().hex[:6]}",
            level="L2", target_mean=Decimal("10.0"), target_sd=Decimal("0.5"),
            expiry_date=datetime.date(2026, 12, 31),
        )
        run = QualityRun.objects.create(
            tenant_id=TENANT, qc=qc,
            run_date=datetime.date.today(), run_time=datetime.time(8, 0),
            technologist_id=PROVIDER, measured_value=Decimal("10.3"),
            z_score=Decimal("0.6"), passed=True,
        )
        assert run.passed is True

    def test_qc_failure_creates_record(self):
        from products.cymed.laboratory.orders.models import LabTest
        from products.cymed.laboratory.quality.models import QualityControl, QualityRun, QualityFailure
        import datetime
        test = LabTest.objects.create(tenant_id=TENANT, code=f"QC-TST-{uuid.uuid4().hex[:4]}", name="QC Test")
        qc = QualityControl.objects.create(
            tenant_id=TENANT, test=test, lot_number=f"LOT-{uuid.uuid4().hex[:6]}",
            level="L1", target_mean=Decimal("5.0"), target_sd=Decimal("0.2"),
            expiry_date=datetime.date(2026, 12, 31),
        )
        run = QualityRun.objects.create(
            tenant_id=TENANT, qc=qc,
            run_date=datetime.date.today(), run_time=datetime.time(9, 0),
            technologist_id=PROVIDER, measured_value=Decimal("5.7"),
            z_score=Decimal("3.5"), passed=False, is_rejection=True,
        )
