...
class TestSpecimen:
    def test_create_specimen(self):
        ...
        sp = Specimen.objects.create(
            ...
            specimen_number="SP-001", barcode="BC-001",
            specimen_type="blood",
        )
    ...
    def test_specimen_rejection(self):
        ...
        sp = Specimen.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            specimen_number="SP-003", barcode="BC-003",
            specimen_type="blood",
        )
        rej = SpecimenRejection.objects.create(
            tenant_id=TENANT, specimen=sp,
            rejection_reason="hemolyzed", rejected_by=PROVIDER,
    ...
    def test_chain_of_custody(self):
        ...
        sp = Specimen.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            specimen_number="SP-004", barcode="BC-004",
            specimen_type="urine",
        )
        now = datetime.datetime.now(tz=datetime.timezone.utc)
        event = SpecimenChainOfCustody.objects.create(
            tenant_id=TENANT, specimen=sp, action="collected",
            performed_by=PROVIDER, location="Ward 3", action_timestamp=now,
        )
    ...
    def test_specimen_transport(self):
        ...
        sp = Specimen.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            specimen_number="SP-005", barcode="BC-005",
            specimen_type="csf",
        )
        now = datetime.datetime.now(tz=datetime.timezone.utc)
        t = SpecimenTransport.objects.create(
            tenant_id=TENANT, specimen=sp,
            from_location="ICU", to_location="Lab",
            dispatched_at=now, status="dispatched",
        )
    ...
    def test_specimen_container(self):
        ...
        sp = Specimen.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            specimen_number="SP-006", barcode="BC-006",
            specimen_type="blood",
        )
        c = SpecimenContainer.objects.create(
            tenant_id=TENANT, specimen=sp,
            container_type="edta", barcode="CONT-001",
        )
