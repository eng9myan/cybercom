...
def make_order_item(tenant_id):
    ...
    test = LabTest.objects.create(tenant_id=tenant_id, code=f"MICRO-{uuid.uuid4().hex[:4]}", name="Culture Test", category="microbiology")
...
def make_specimen(tenant_id):
    ...
    return Specimen.objects.create(
        tenant_id=tenant_id, patient_id=PATIENT,
...
class TestCulture:
    def test_create_culture(self):
        ...
        culture = Culture.objects.create(
            tenant_id=TENANT, order_item=item, specimen=sp,
            culture_number=f"CUL-{uuid.uuid4().hex[:8]}",
            culture_type="aerobic", medium="Blood Agar",
    ...
    def test_organism_identification(self):
        ...
        culture = Culture.objects.create(
            tenant_id=TENANT, order_item=item, specimen=sp,
            culture_number=f"CUL-{uuid.uuid4().hex[:8]}", culture_type="aerobic",
        )
        organism = Organism.objects.create(
            tenant_id=TENANT, culture=culture,
            organism_name="Staphylococcus aureus",
            snomed_code="3092008", gram_stain="gram_positive",
            morphology="cocci", growth_level="heavy",
        )
        assert organism.gram_stain == "gram_positive"
        assert culture.organisms.count() == 1

    def test_antibiotic_sensitivity(self):
        from products.cymed.laboratory.microbiology.models import Culture, Organism, Sensitivity, SensitivityResult
        item = make_order_item(TENANT)
        sp = make_specimen(TENANT)
        culture = Culture.objects.create(
            tenant_id=TENANT, order_item=item, specimen=sp,
        ...
        organism = Organism.objects.create(
            tenant_id=TENANT, culture=culture,
            organism_name="E. coli", gram_stain="gram_negative",
        )
        sens = Sensitivity.objects.create(
            tenant_id=TENANT, organism=organism,
            antibiotic_code="AMX", antibiotic_name="Amoxicillin",
            method="disk_diffusion", result=SensitivityResult.RESISTANT,
            mic_value=">32",
        )
        assert sens.result == "R"

    def test_mrsa_resistance_profile(self):
        from products.cymed.laboratory.microbiology.models import Culture, Organism, ResistanceProfile
        item = make_order_item(TENANT)
        sp = make_specimen(TENANT)
        culture = Culture.objects.create(
            tenant_id=TENANT, order_item=item, specimen=sp,
        ...
        organism = Organism.objects.create(
            tenant_id=TENANT, culture=culture,
            organism_name="Staphylococcus aureus", gram_stain="gram_positive",
        )
        profile = ResistanceProfile.objects.create(
            tenant_id=TENANT, organism=organism,
            resistance_mechanism="mrsa", confirmed=True,
        )
