...
class TestAnalyzer:
    def test_create_analyzer(self):
        ...
        a = Analyzer.objects.create(
            tenant_id=TENANT, code="CHEM-001", name="Cobas c702",
            manufacturer="Roche", model_number="c702",
            analyzer_type="chemistry", department="chemistry",
        )
    ...
    def test_analyzer_interface(self):
        ...
        analyzer = Analyzer.objects.create(
            tenant_id=TENANT, code="CHEM-002", name="Analyzer 2",
            analyzer_type="chemistry", department="chemistry",
        )
        test = LabTest.objects.create(tenant_id=TENANT, code=f"GLU-{uuid.uuid4().hex[:4]}", name="Glucose")
        iface = AnalyzerInterface.objects.create(
            tenant_id=TENANT, analyzer=analyzer, test=test,
            instrument_test_code="GLU", is_primary=True,
        )
    ...
    def test_analyzer_message(self):
        ...
        analyzer = Analyzer.objects.create(
            tenant_id=TENANT, code="HEMA-001", name="XN-9000",
            analyzer_type="hematology", department="hematology",
        )
        msg = AnalyzerMessage.objects.create(
            tenant_id=TENANT, analyzer=analyzer,
            direction="inbound", message_type="ORU",
...
class TestWorklist:
    def test_create_worklist(self):
        ...
        wl = LabWorklist.objects.create(
            tenant_id=TENANT, name="Morning Chemistry",
            department="chemistry", worklist_date=datetime.date.today(),
            created_by=PROVIDER,
        )
    ...
    def test_worklist_item(self):
        ...
        test = LabTest.objects.create(tenant_id=TENANT, code=f"ALT-{uuid.uuid4().hex[:4]}", name="ALT")
        ...
        wl = LabWorklist.objects.create(
            tenant_id=TENANT, name="Test WL", department="chemistry",
            worklist_date=datetime.date.today(), created_by=PROVIDER,
        )
        wi = WorklistItem.objects.create(
            tenant_id=TENANT, worklist=wl, order_item=item, sequence=1,
        )
    ...
    def test_technologist_assignment(self):
        ...
        wl = LabWorklist.objects.create(
            tenant_id=TENANT, name="Afternoon WL", department="hematology",
            worklist_date=datetime.date.today(), created_by=PROVIDER,
        )
        assign = TechnologistAssignment.objects.create(
            tenant_id=TENANT, worklist=wl, technologist_id=PROVIDER,
            assigned_by=PROVIDER, items_target=50,
        )
...
class TestAccessioning:
    def test_accession_number_sequence(self):
        ...
        seq = AccessionNumberSequence.objects.create(
            tenant_id=TENANT, site_code="HQ", year=2026,
            prefix="ACC", last_sequence=0,
        )
    ...
    def test_accession_creation(self):
        ...
        sp = Specimen.objects.create(
            ...
            specimen_number=f"SP-{uuid.uuid4().hex[:6]}", barcode=f"BC-{uuid.uuid4().hex[:6]}",
            specimen_type="blood",
        )
        acc = Accession.objects.create(
            tenant_id=TENANT, specimen=sp,
            accession_number="ACC-HQ-2026-000001",
            site_code="HQ", accessioned_by=PROVIDER,
        )
    ...
    def test_accession_batch(self):
        ...
        batch = AccessionBatch.objects.create(
            ...
            site_code="HQ", created_by=PROVIDER,
        )
