...
from products.cymed.core.patients.models import (
    Patient, PatientIdentifier, PatientContact, PatientAddress, 
    PatientCommunication, PatientMergeHistory
)
...
class PatientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Patient
        fields = [
            "id", "first_name", "last_name", "dob", "gender", "mrn",
            "national_id", "passport_number", "is_active", "merged_into",
            "identifiers", "contacts", "addresses", "communications", "created_at", "updated_at"
        ]
    ...
    def create(self, validated_data):
        ...
        for id_data in identifiers_data:
            PatientIdentifier.objects.create(patient=patient, tenant_id=patient.tenant_id, **id_data)
        for contact_data in contacts_data:
            PatientContact.objects.create(patient=patient, tenant_id=patient.tenant_id, **contact_data)
        ...
        for comm_data in communications_data:
            PatientCommunication.objects.create(patient=patient, tenant_id=patient.tenant_id, **comm_data)
