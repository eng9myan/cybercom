...
class ConsentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Consent
        fields = ["id", "patient", "category", "status", "policy_rule", "start_time", "end_time", "signature", "created_at", "updated_at"]
    ...
    def create(self, validated_data):
        ...
        if sig_data:
            ConsentSignature.objects.create(consent=consent, tenant_id=consent.tenant_id, **sig_data)
        ...
        OutboxEvent.objects.create(
            ...
            payload={"consent_id": str(consent.id), "patient_id": str(consent.patient.id), "category": consent.category}
        )
