...
from products.cymed.core.providers.models import (
    Provider, ProviderSpecialty, ProviderRole, ProviderAvailability, ProviderLicense
)
...
class ProviderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Provider
        fields = [
            "id", "user_id", "first_name", "last_name", "provider_type",
            "npi", "is_active", "specialties", "roles", "availability", "licenses", "created_at", "updated_at"
        ]
    ...
    def create(self, validated_data):
        ...
        for spec in specialties_data:
            ProviderSpecialty.objects.create(provider=provider, tenant_id=provider.tenant_id, **spec)
        for role in roles_data:
            ProviderRole.objects.create(provider=provider, tenant_id=provider.tenant_id, **role)
        for avail in availability_data:
            ProviderAvailability.objects.create(provider=provider, tenant_id=provider.tenant_id, **avail)
        ...
        OutboxEvent.objects.create(
            ...
            payload={"provider_id": str(provider.id), "npi": provider.npi, "type": provider.provider_type}
        )
