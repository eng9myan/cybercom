...
class TestProviderWorkspace:
    ...
    def test_provider_dashboard(self):
        from products.cymed.provider_portal.workspace.models import ProviderWorkspace, ProviderDashboard
        ws = ProviderWorkspace.objects.create(
        ...
        dashboard = ProviderDashboard.objects.create(
            tenant_id=TENANT, workspace=ws,
            layout_config={"tasks": {"x": 0, "y": 0, "w": 6, "h": 4}},
            show_tasks=True, show_results=True,
            show_census=True,
        )
    ...
    def test_provider_preferences(self):
        from products.cymed.provider_portal.workspace.models import ProviderWorkspace, ProviderPreferences
        ws = ProviderWorkspace.objects.create(
            tenant_id=TENANT, provider_id=uuid.uuid4(),
            provider_type="pharmacist", cyidentity_user_id=uuid.uuid4(),
        )
    ...
    def test_workspace_session(self):
        from products.cymed.provider_portal.workspace.models import ProviderWorkspace, WorkspaceSession
        ws = ProviderWorkspace.objects.create(
            tenant_id=TENANT, provider_id=uuid.uuid4(),
            provider_type="radiologist", cyidentity_user_id=uuid.uuid4(),
        )
    ...
    def test_all_provider_types(self):
        from products.cymed.provider_portal.workspace.models import ProviderWorkspace
        provider_types = [
            "physician", "consultant", "resident", "nurse", "charge_nurse",
            "pharmacist", "clinical_pharmacist", "radiologist", "lab_technologist",
            "microbiologist", "pathologist", "therapist", "care_coordinator", "administrator",
        ]
        for i, ptype in enumerate(provider_types):
            ws = ProviderWorkspace.objects.create(
                tenant_id=TENANT, provider_id=uuid.uuid4(),
                provider_type=ptype, cyidentity_user_id=uuid.uuid4(),
            )
...
class TestPatientLists:
    ...
    def test_patient_census(self):
        ...
        census = PatientCensus.objects.create(
            ...
            total_beds=30, occupied_beds=24, available_beds=6,
            pending_admissions=2, pending_discharges=3,
            average_acuity=3.2,
        )
    ...
    def test_icu_patient_list(self):
        ...
        icu = PatientList.objects.create(
            tenant_id=TENANT, name="ICU Patients",
            list_type="icu", workspace_id=uuid.uuid4(),
            unit_id=UNIT_ID, auto_populate=True,
        )
...
class TestClinicalTasks:
    ...
    def test_task_assignment(self):
        from products.cymed.provider_portal.clinical_tasks.models import ClinicalTask, TaskAssignment
        task = ClinicalTask.objects.create(
            tenant_id=TENANT, task_type="wound_care",
            title="Wound care for patient bed 4B-12",
            patient_id=PATIENT, priority="routine", status="pending",
            assigned_to_provider_id=PROVIDER,
        ...
        ta = TaskAssignment.objects.create(
            ...
            provider_id=PROVIDER, provider_type="nurse",
    ...
    def test_task_comment(self):
        ...
        task = ClinicalTask.objects.create(
            tenant_id=TENANT, task_type="medication_review",
            title="Medication reconciliation on admission",
            patient_id=PATIENT, priority="urgent", status="in_progress",
            assigned_to_provider_id=PROVIDER,
    ...
    def test_task_escalation(self):
        from products.cymed.provider_portal.clinical_tasks.models import ClinicalTask, TaskEscalation
        from django.utils import timezone
        task = ClinicalTask.objects.create(
            tenant_id=TENANT, task_type="patient_callback",
            title="Urgent patient callback - deteriorating vitals",
            patient_id=PATIENT, priority="urgent", status="escalated",
            assigned_to_provider_id=PROVIDER,
            assigned_to_type="physician",
            created_by_provider_id=uuid.uuid4(),
        )
...
class TestClinicalMessaging:
    ...
    def test_send_clinical_message(self):
        from products.cymed.provider_portal.clinical_messaging.models import (
            ClinicalMessageThread, ClinicalMessage
        )
        thread = ClinicalMessageThread.objects.create(
            tenant_id=TENANT, thread_type="direct",
            subject="Quick question about anticoagulation",
            created_by_provider_id=PROVIDER, status="active",
        )
    ...
    def test_clinical_group(self):
        ...
        group = ClinicalGroup.objects.create(
            ...
            group_type="on_call",
            members=[
                {"provider_id": str(uuid.uuid4()), "provider_name": "Dr. Hassan", "role": "attending"},
                {"provider_id": str(uuid.uuid4()), "provider_name": "Dr. Layla", "role": "resident"},
            ],
    ...
    def test_message_participant(self):
        from products.cymed.provider_portal.clinical_messaging.models import (
            ClinicalMessageThread, MessageThreadParticipant
        )
        thread = ClinicalMessageThread.objects.create(
            tenant_id=TENANT, thread_type="team",
            subject="ICU Handoff",
            created_by_provider_id=PROVIDER, status="active",
        )
...
class TestRounding:
    def test_create_clinical_round(self):
        ...
        rnd = ClinicalRound.objects.create(
            ...
            status="scheduled", patient_count=12,
        )
    ...
    def test_round_team(self):
        ...
        from django.utils import timezone
        rnd = ClinicalRound.objects.create(
            tenant_id=TENANT, round_type="multidisciplinary",
            unit_id=UNIT_ID, unit_name="ICU",
        ...
        team_member = RoundTeam.objects.create(
            tenant_id=TENANT, round=rnd,
            provider_id=uuid.uuid4(), provider_name="Dr. Layla Resident",
            provider_type="physician", role="resident",
            is_present=True,
        )
    ...
    def test_round_finding_and_action(self):
        ...
        rnd = ClinicalRound.objects.create(
            tenant_id=TENANT, round_type="icu",
            unit_id=UNIT_ID, unit_name="ICU",
            attending_provider_id=PROVIDER, attending_name="Dr. Hassan",
            round_date=date.today(), status="in_progress",
        )
