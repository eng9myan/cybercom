...
class TestClinicalDocumentation:
    def test_create_documentation_template(self):
        from products.cymed.provider_portal.clinical_documentation.models import DocumentationTemplate
    ...
    def test_note_cosignature(self):
        from products.cymed.provider_portal.clinical_documentation.models import ProviderClinicalNote, NoteCoSignature
        note = ProviderClinicalNote.objects.create(
    ...
    def test_ai_note_summary_advisory_only(self):
        ...
        note = ProviderClinicalNote.objects.create(
            ...
            author_type="physician", note_type="discharge",
            note_body="Discharge note body.", status="draft",
...
class TestOrders:
    ...
    def test_order_modification(self):
        from products.cymed.provider_portal.orders.models import ProviderOrderRequest, OrderModification
        order = ProviderOrderRequest.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            ordering_provider_id=PROVIDER, ordering_provider_name="Dr. Hassan",
            order_category="laboratory", order_name="Troponin",
            priority="routine", status="submitted",
        )
        mod = OrderModification.objects.create(
            tenant_id=TENANT, order=order,
...
class TestResults:
    ...
    def test_critical_result_alert(self):
        ...
        result = ProviderResultView.objects.create(
            ...
            result_date=date.today(), result_status="final",
            is_critical=True, ordering_provider_id=PROVIDER,
        )
    ...
    def test_result_acknowledgement(self):
        from products.cymed.provider_portal.results.models import ProviderResultView, ResultAcknowledgement
        result = ProviderResultView.objects.create(
            tenant_id=TENANT, patient_id=PATIENT,
            result_type="imaging",
            result_source_id=uuid.uuid4(),
...
class TestCareTeam:
    ...
    def test_add_care_team_members(self):
        ...
        team = CareTeam.objects.create(
            tenant_id=TENANT, team_name="ICU Team",
            team_type="multidisciplinary", unit_id=UNIT_ID,
            created_by_provider_id=PROVIDER,
        )
        attending = CareTeamMember.objects.create(
            tenant_id=TENANT, care_team=team,
            provider_id=PROVIDER, provider_name="Dr. Hassan",
            provider_type="physician", role="attending",
            is_primary=True, is_active=True, added_by=PROVIDER,
        )
        pharmacist = CareTeamMember.objects.create(
            tenant_id=TENANT, care_team=team,
            provider_id=uuid.uuid4(), provider_name="PharmD Fatima",
            provider_type="clinical_pharmacist", role="pharmacist",
            is_primary=False, is_active=True, added_by=PROVIDER,
        )
    ...
    def test_care_team_assignment(self):
        ...
        team = CareTeam.objects.create(
            tenant_id=TENANT, team_name="MDT Team",
    ...
    def test_coverage_schedule(self):
        ...
        team = CareTeam.objects.create(
            tenant_id=TENANT, team_name="On-Call Team",
            team_type="on_call", created_by_provider_id=PROVIDER,
        )
...
class TestApprovals:
    ...
    def test_approve_request(self):
        from products.cymed.provider_portal.approvals.models import ApprovalRequest, ApprovalDecision
        req = ApprovalRequest.objects.create(
            tenant_id=TENANT,
        ...
        decision = ApprovalDecision.objects.create(
            tenant_id=TENANT, approval_request=req,
    ...
    def test_approval_audit_log(self):
        from products.cymed.provider_portal.approvals.models import ApprovalRequest, ApprovalAuditLog
        req = ApprovalRequest.objects.create(
            tenant_id=TENANT,
            approval_type="administrative",
            title="Schedule change request",
            requested_by_provider_id=PROVIDER, requested_by_name="Dr. Hassan",
            approver_id=PROVIDER_2, approver_name="Admin",
            status="pending",
        )
...
class TestProviderTelemedicine:
    ...
    def test_second_opinion_request(self):
        ...
        opinion = SecondOpinionRequest.objects.create(
            ...
            status="pending",
            attached_records=[
                {"record_type": "imaging", "record_id": str(uuid.uuid4()), "record_title": "CT Chest"},
                {"record_type": "lab_result", "record_id": str(uuid.uuid4()), "record_title": "LDH Level"},
            ],
        )
