...
def make_order_item(tenant_id):
    ...
    proc = ImagingProcedure.objects.create(
        tenant_id=tenant_id, code=f"PROC-{uuid.uuid4().hex[:4]}", name="MRI Brain", modality="mri",
    )
...
def make_report(tenant_id):
    ...
    return RadiologyReport.objects.create(
        tenant_id=tenant_id, order_item=item,
...
def make_modality(tenant_id):
    ...
    return Modality.objects.create(
        tenant_id=tenant_id, code=f"MOD-{uuid.uuid4().hex[:4]}",
        name="CT Scanner", modality_type="ct",
    )
...
class TestTeleradiology:
    def test_reading_queue(self):
        ...
        queue = ReadingQueue.objects.create(
            tenant_id=TENANT, name="Neuroradiology Queue",
            queue_type="subspecialty", subspecialty="neuroradiology",
    ...
    def test_teleradiology_case(self):
        from products.cymed.imaging.teleradiology.models import TeleradiologyCase, ReadingQueue
        queue = ReadingQueue.objects.create(
            tenant_id=TENANT, name="Urgent Queue",
            queue_type="urgent", max_turnaround_hours=1,
        )
        item = make_order_item(TENANT)
        case = TeleradiologyCase.objects.create(
            tenant_id=TENANT, order_item=item, reading_queue=queue,
            case_type="primary", originating_facility="Al-Noor Hospital",
            target_turnaround_hours=6, priority="stat",
        )
        assert case.status == "pending"
        assert case.priority == "stat"

    def test_reading_assignment(self):
        from products.cymed.imaging.teleradiology.models import TeleradiologyCase, ReadingAssignment
        item = make_order_item(TENANT)
        case = TeleradiologyCase.objects.create(
            tenant_id=TENANT, order_item=item, case_type="primary",
        )
        assignment = ReadingAssignment.objects.create(
            tenant_id=TENANT, teleradiology_case=case,
            radiologist_id=RAD, assigned_by=PROVIDER,
    ...
    def test_second_opinion_with_discrepancy(self):
        ...
        opinion = SecondOpinion.objects.create(
            tenant_id=TENANT, original_report=report,
            requested_by=PROVIDER, reason="Review",
...
class TestQuality:
    def test_quality_audit(self):
        ...
        audit = QualityAudit.objects.create(
            ...
            auditor_id=RAD2, audit_type="peer_review",
            score=4, feedback="Well-structured report.",
    ...
    def test_quality_audit_discrepancy(self):
        ...
        audit = QualityAudit.objects.create(
            tenant_id=TENANT, report=report,
            auditor_id=RAD2, audit_type="peer_review",
            score=2, discrepancy_level="major",
    ...
    def test_radiation_dose_record(self):
        ...
        dose = RadiationDoseRecord.objects.create(
            ...
            patient_id=PATIENT, modality="ct",
            ctdivol=Decimal("8.5"), dlp=Decimal("425.0"),
    ...
    def test_accreditation_record(self):
        ...
        acc = AccreditationRecord.objects.create(
            tenant_id=TENANT,
            accreditation_body="ACR", accreditation_type="CT Accreditation",
