...
def make_order_item(tenant_id):
    ...
    proc = ImagingProcedure.objects.create(
        tenant_id=tenant_id, code=f"PROC-{uuid.uuid4().hex[:4]}", name="X-Ray", modality="xray",
    )
...
class TestAnalytics:
    def test_ops_dashboard(self):
        ...
        dash = ImagingOperationsDashboard.objects.create(
            ...
            total_orders=200, studies_completed=195,
            reports_finalized=190, critical_findings=2,
    ...
    def test_radiologist_productivity(self):
        ...
        prod = RadiologistProductivity.objects.create(
            tenant_id=TENANT, radiologist_id=RAD,
            period_start=datetime.date.today(),
            period_end=datetime.date.today(),
            period_type="daily",
            total_rvu=Decimal("45.5"),
            studies_read=32, reports_finalized=30,
    ...
    def test_teleradiology_dashboard(self):
        ...
        dash = TeleradiologyDashboard.objects.create(
            tenant_id=TENANT, snapshot_date=datetime.date.today(),
            total_cases=50, cases_completed=48,
    ...
    def test_imaging_revenue_event(self):
        ...
        event = ImagingRevenueEvent.objects.create(
            tenant_id=TENANT, order_item_id=item_id,
            patient_id=PATIENT, procedure_code="CT-CHEST",
            modality="ct", rvu_value=Decimal("3.52"),
...
class TestFHIRMapping:
    def test_fhir_service_request(self):
        ...
        order = ImagingOrder.objects.create(
            ...
            order_number="IMG-FHIR-001", priority="stat",
            order_type="outpatient", status="pending",
    ...
    def test_fhir_diagnostic_report(self):
        ...
        result = ImagingResult.objects.create(
            tenant_id=TENANT, order_item=item, status="final",
            result_date=datetime.datetime.now(tz=datetime.timezone.utc),
        )
    ...
    def test_fhir_imaging_study(self):
        ...
        pacs = PACSNode.objects.create(
            tenant_id=TENANT, code=f"PACS-FHIR-{uuid.uuid4().hex[:4]}",
            name="PACS", ae_title="P1", host="10.0.0.1",
        )
        study = DICOMStudy.objects.create(
            ...
            series_count=3, instance_count=150,
        )
...
class TestImagingOrderService:
    def test_create_order(self):
        ...
        ImagingProcedure.objects.create(
            tenant_id=TENANT, code="CT-HEAD-SVC",
            name="CT Head", modality="ct", is_active=True,
        )
        result = ImagingOrderService.create_order(
            tenant_id=TENANT, patient_id=PATIENT,
            order_type="outpatient", procedures=["CT-HEAD-SVC"],
            priority="routine", ordered_by=PROVIDER,
...
class TestEditionFeatureMaps:
    ...
    def test_imaging_enterprise_superset_of_basic(self):
        from products.cymed.commercial.feature_flags.services import (
            IMAGING_BASIC_FEATURES, IMAGING_ENTERPRISE_FEATURES,
        )
    ...
    def test_imaging_teleradiology_superset_of_enterprise(self):
        from products.cymed.commercial.feature_flags.services import (
            IMAGING_ENTERPRISE_FEATURES, IMAGING_TELERADIOLOGY_FEATURES,
        )
    ...
    def test_imaging_national_superset_of_teleradiology(self):
        from products.cymed.commercial.feature_flags.services import (
            IMAGING_TELERADIOLOGY_FEATURES, IMAGING_NATIONAL_FEATURES,
        )
