...
def make_order_item(tenant_id):
    ...
    proc = ImagingProcedure.objects.create(
        tenant_id=tenant_id, code=f"PROC-{uuid.uuid4().hex[:4]}", name="Test Proc", modality="ct",
    )
...
class TestModality:
    def test_create_modality(self):
        ...
        mod = Modality.objects.create(
            tenant_id=TENANT, code="CT-001", name="Siemens SOMATOM",
            modality_type="ct", manufacturer="Siemens",
            ae_title="CT001", ip_address="192.168.1.100",
        )
    ...
    def test_worklist_entry(self):
        ...
        entry = WorklistEntry.objects.create(
            tenant_id=TENANT, worklist=wl, order_item=item,
            patient_id=PATIENT, priority_rank=1,
        )
    ...
    def test_study_queue(self):
        ...
        queue = StudyQueue.objects.create(
            tenant_id=TENANT, order_item=item, queue_type="reading", position=1,
        )
...
class TestScheduling:
    def test_imaging_room(self):
        ...
        room = ImagingRoom.objects.create(
            tenant_id=TENANT, code="CT-ROOM-1", name="CT Suite 1",
            modality_type="ct", building="A", floor="2",
        )
