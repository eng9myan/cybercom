...
class TestImagingProtocol:
    def test_create_protocol(self):
        ...
        proto = ImagingProtocol.objects.create(
            tenant_id=TENANT, name="CT Chest Protocol",
            modality="ct", description="Standard chest CT",
    ...
    def test_create_procedure(self):
        ...
        proc = ImagingProcedure.objects.create(
            tenant_id=TENANT, code="CT-CHEST", name="CT Chest",
            modality="ct", body_part="chest",
            snomed_code="399208008", loinc_code="24627-2",
            duration_minutes=15, contrast_required=False,
        )
        assert proc.code == "CT-CHEST"
        assert proc.is_active is True

    def test_procedure_with_protocol(self):
        from products.cymed.imaging.orders.models import ImagingProtocol, ImagingProcedure
        proto = ImagingProtocol.objects.create(tenant_id=TENANT, name="MRI Brain", modality="mri")
        proc = ImagingProcedure.objects.create(
            tenant_id=TENANT, code=f"MRI-BRAIN-{uuid.uuid4().hex[:4]}",
            name="MRI Brain", modality="mri", protocol=proto,
            contrast_required=True, duration_minutes=45,
        )
    ...
    def test_create_imaging_order(self):
        ...
        order = ImagingOrder.objects.create(
            ...
            order_type="outpatient", priority="routine",
    ...
    def test_imaging_order_item(self):
        ...
        proc = ImagingProcedure.objects.create(
            tenant_id=TENANT, code=f"XR-{uuid.uuid4().hex[:4]}",
            name="Chest X-Ray", modality="xray",
        )
        order = ImagingOrder.objects.create(
            tenant_id=TENANT, patient_id=PATIENT, ordered_by=PROVIDER,
            order_number=f"IMG-{uuid.uuid4().hex[:8].upper()}",
        ...
        item = ImagingOrderItem.objects.create(
            tenant_id=TENANT, order=order, procedure=proc,
            body_part="chest", laterality="none",
        )
    ...
    def test_order_status_history(self):
        ...
        history = ImagingOrderStatusHistory.objects.create(
            ...
            from_status="", to_status="pending", changed_by=PROVIDER,
        )
