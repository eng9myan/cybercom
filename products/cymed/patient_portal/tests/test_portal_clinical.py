...
class TestLabResults:
    ...
    def test_critical_lab_result(self):
        ...
        result = LabResultView.objects.create(
            ...
            lab_name="City Lab", test_name="Potassium",
            loinc_code="2823-3", result_status="final",
            is_critical=True,
        )
        assert result.is_critical is True

    def test_critical_result_acknowledgement(self):
        from products.cymed.patient_portal.laboratory_results.models import LabResultView, CriticalResultAcknowledgement
        result = LabResultView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_lab_result_id=uuid.uuid4(), lab_id=uuid.uuid4(),
            lab_name="City Lab", test_name="Sodium",
            result_status="final", is_critical=True,
        )
        ack = CriticalResultAcknowledgement.objects.create(
            tenant_id=TENANT, lab_result=result,
            account_id=ACCOUNT, patient_id=PATIENT,
    ...
    def test_lab_result_trend(self):
        ...
        trend = LabResultTrend.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            test_code="2345-7", test_name="HbA1c",
            loinc_code="4548-4", unit="%",
    ...
    def test_lab_share_link(self):
        from products.cymed.patient_portal.laboratory_results.models import LabResultView, LabResultShareLink
        result = LabResultView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_lab_result_id=uuid.uuid4(), lab_id=uuid.uuid4(),
            lab_name="Test Lab", test_name="Lipid Panel",
            result_status="final",
        )
        link = LabResultShareLink.objects.create(
            tenant_id=TENANT, lab_result=result, account_id=ACCOUNT,
...
class TestImagingResults:
    ...
    def test_imaging_study_metadata(self):
        from products.cymed.patient_portal.imaging_results.models import ImagingResultView, ImagingStudyMetadata
        result = ImagingResultView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_imaging_result_id=uuid.uuid4(),
            imaging_center_id=uuid.uuid4(),
        ...
        series = ImagingStudyMetadata.objects.create(
            tenant_id=TENANT, imaging_result=result,
            series_number=1, series_description="Axial T1",
            modality="MRI", instance_count=120,
        )
        assert series.instance_count == 120
        assert result.series.count() == 1

    def test_imaging_report_access(self):
        from products.cymed.patient_portal.imaging_results.models import ImagingResultView, ImagingReportAccess
        result = ImagingResultView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_imaging_result_id=uuid.uuid4(),
            imaging_center_id=uuid.uuid4(),
            imaging_center_name="Test Center",
            modality="X-Ray", report_status="final",
        )
        access = ImagingReportAccess.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            imaging_result=result, access_type="download_report",
        )
        assert access.access_type == "download_report"
        assert result.access_log.count() == 1

    def test_imaging_share_link(self):
        from products.cymed.patient_portal.imaging_results.models import ImagingResultView, ImagingShareLink
        result = ImagingResultView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_imaging_result_id=uuid.uuid4(),
            imaging_center_id=uuid.uuid4(),
        ...
        link = ImagingShareLink.objects.create(
            tenant_id=TENANT, imaging_result=result, account_id=ACCOUNT,
...
class TestPortalPrescriptions:
    ...
    def test_refill_request(self):
        from products.cymed.patient_portal.prescriptions.models import PortalPrescriptionView, RefillRequest
        rx = PortalPrescriptionView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_prescription_id=uuid.uuid4(),
            prescription_number="RX-PORTAL-002",
            status="active", can_request_refill=True,
        )
    ...
    def test_medication_adherence(self):
        from products.cymed.patient_portal.prescriptions.models import PortalPrescriptionView, MedicationAdherenceLog
        rx = PortalPrescriptionView.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            cymed_prescription_id=uuid.uuid4(),
...
class TestHealthJourney:
    ...
    def test_timeline_events(self):
        from products.cymed.patient_portal.health_journey.models import HealthTimeline, HealthTimelineEvent
        timeline = HealthTimeline.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
        )
        events = [
            HealthTimelineEvent.objects.create(
                tenant_id=TENANT, timeline=timeline,
                account_id=ACCOUNT, patient_id=PATIENT,
                event_type=etype, title=f"Event: {etype}",
                provider_name="Dr. Hassan",
                facility_name="City Hospital",
                event_date=date.today() - timedelta(days=i * 30),
            )
            for i, etype in enumerate(["appointment", "lab_result", "prescription", "hospitalization"])
        ]
    ...
    def test_patient_journey(self):
        ...
        journey = PatientJourney.objects.create(
            ...
            status="active",
            care_team=[{"provider_id": str(uuid.uuid4()), "provider_name": "Dr. Hassan", "role": "Endocrinologist"}],
        )
        assert journey.status == "active"
        assert journey.journey_type == "chronic_condition"

    def test_health_milestone(self):
        from products.cymed.patient_portal.health_journey.models import PatientJourney, HealthMilestone
        journey = PatientJourney.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
        ...
        m = HealthMilestone.objects.create(
            tenant_id=TENANT, journey=journey,
            account_id=ACCOUNT, patient_id=PATIENT,
...
class TestPayments:
    ...
    def test_payment_transaction(self):
        ...
        invoice = PatientInvoice.objects.create(
            ...
            amount_total=75.00, amount_patient_due=75.00,
    ...
    def test_installment_plan(self):
        from products.cymed.patient_portal.payments.models import PatientInvoice, InstallmentPlan
        invoice = PatientInvoice.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            ...
            amount_total=5000.00, amount_patient_due=1000.00,
            currency="USD", status="pending",
        )
        plan = InstallmentPlan.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            invoice=invoice,
            total_amount=1000.00, installment_count=4,
...
class TestInsurance:
    def test_insurance_card(self):
        ...
        card = InsuranceCard.objects.create(
            ...
            deductible=500.00, out_of_pocket_max=3000.00,
        )
        assert card.is_primary is True
        assert card.plan_type == "individual"

    def test_coverage_verification(self):
        from products.cymed.patient_portal.insurance.models import InsuranceCard, CoverageVerification
        card = InsuranceCard.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
    ...
    def test_preauthorization_request(self):
        from products.cymed.patient_portal.insurance.models import InsuranceCard, PreauthorizationRequest
        card = InsuranceCard.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            insurer_name="Test Insurer", policy_number="POL-003", member_id="MEM-003",
        )
    ...
    def test_claim_status(self):
        from products.cymed.patient_portal.insurance.models import InsuranceCard, ClaimStatus
        card = InsuranceCard.objects.create(
            tenant_id=TENANT, account_id=ACCOUNT, patient_id=PATIENT,
            insurer_name="Test Insurer", policy_number="POL-004", member_id="MEM-004",
        )
...
class TestDigitalWallet:
    ...
    def test_digital_card(self):
        ...
        card = DigitalCard.objects.create(
            tenant_id=TENANT, wallet_account_id=ACCOUNT,
            card_type="insurance",
    ...
    def test_vaccination_record(self):
        ...
        vax = VaccinationRecord.objects.create(
            ...
            lot_number="EJ1685",
            dose_number=2, total_doses_required=2,
