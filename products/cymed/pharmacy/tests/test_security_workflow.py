...
class TestTenantIsolation:
    ...
    def test_prescription_tenant_isolation(self):
        ...
        t1 = uuid.uuid4()
        t2 = uuid.uuid4()
        Prescription.objects.create(tenant_id=t1, prescription_number="RX-T1-001", patient_id=PATIENT, prescriber_id=PROVIDER)
        Prescription.objects.create(tenant_id=t2, prescription_number="RX-T2-001", patient_id=PATIENT, prescriber_id=PROVIDER)
    ...
    def test_interaction_rule_tenant_isolation(self):
        ...
        InteractionRule.objects.create(
            tenant_id=t1, rule_code="DDI-T1-001",
            interaction_type="drug_drug", severity="moderate",
            drug_a_code="RXN-A", drug_a_name="Drug A",
            drug_b_code="RXN-B", drug_b_name="Drug B",
            description="Test", management_recommendation="Test",
        )
        InteractionRule.objects.create(
            tenant_id=t2, rule_code="DDI-T2-001",
            interaction_type="drug_drug", severity="mild",
            drug_a_code="RXN-C", drug_a_name="Drug C",
            drug_b_code="RXN-D", drug_b_name="Drug D",
            description="Test2", management_recommendation="Test2",
        )
...
class TestControlledSubstanceTracking:
    ...
    def test_controlled_prescription_fields(self):
        ...
        rx = Prescription.objects.create(
            tenant_id=TENANT, prescription_number="CS-001",
            patient_id=PATIENT, prescriber_id=PROVIDER,
            prescriber_dea="AB1234567", is_controlled=True,
    ...
    def test_controlled_medication_order(self):
        from products.cymed.pharmacy.prescriptions.models import MedicationOrder, DEASchedule
        order = MedicationOrder.objects.create(
            tenant_id=TENANT, order_number="MO-CS-001",
            patient_id=PATIENT, admission_id=uuid.uuid4(),
            prescriber_id=PROVIDER, drug_code="RXN-MORPH",
            drug_name="Morphine 10mg", dose="10", dose_unit="mg",
            route="IV", frequency="Q4H PRN",
    ...
    def test_dispense_audit_for_controlled(self):
        ...
        order = DispenseOrder.objects.create(
            tenant_id=TENANT, dispense_number="DSP-CS-001", patient_id=PATIENT,
        )
        audit = DispenseAudit.objects.create(
            tenant_id=TENANT, dispense_order=order, action="dispensed",
            performed_by=PHARMACIST, is_override=False,
...
class TestMedicationWorkflow:
    ...
    def test_complete_medication_workflow(self):
        ...
        """
        from products.cymed.pharmacy.prescriptions.models import Prescription, PrescriptionItem, PrescriptionStatus
        ...
        rx = Prescription.objects.create(
            tenant_id=TENANT, prescription_number="RX-FLOW-001",
            patient_id=PATIENT, prescriber_id=PROVIDER,
            prescription_type="outpatient", status=PrescriptionStatus.PENDING,
        )
        ...
        dispense = DispenseOrder.objects.create(
            tenant_id=TENANT, dispense_number="DSP-FLOW-001",
            prescription_id=rx.id, patient_id=PATIENT,
            dispense_type="retail", status=DispenseStatus.VERIFICATION_PENDING,
        )
        DispenseItem.objects.create(
            tenant_id=TENANT, dispense_order=dispense,
            drug_code="RXN-001", drug_name="Amoxicillin 500mg",
...
class TestPharmacyEventPublishing:
    ...
    def test_prescription_event_not_duplicate(self):
        ...
        rx = Prescription.objects.create(
            tenant_id=TENANT, prescription_number="RX-EVT-001",
            patient_id=PATIENT, prescriber_id=PROVIDER,
        )
        dispense = DispenseOrder.objects.create(
            tenant_id=TENANT, dispense_number="DSP-EVT-001",
            prescription_id=rx.id, patient_id=PATIENT,
        )
...
class TestPharmacySecurityRoles:
    ...
    def test_ai_cannot_set_verified_status(self):
        ...
        rx = Prescription.objects.create(
            tenant_id=TENANT, prescription_number="RX-SEC-001",
            patient_id=PATIENT, prescriber_id=PROVIDER,
        ...
        review = MedicationReview.objects.create(
            ...
            review_type="dur", ai_polypharmacy_score=0.85,
        )
    ...
    def test_override_requires_reason(self):
        ...
        from products.cymed.pharmacy.drug_interactions.serializers import InteractionOverrideSerializer
