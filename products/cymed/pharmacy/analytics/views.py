...
from .serializers import (
    PharmacyDashboardSnapshotSerializer, MedicationSafetyEventSerializer, PharmacyAnalyticsConfigSerializer
)
    ...
    def get(self, request):
        ...
        return Response({
            ...
            "prescriptions": {
                ...
                "controlled": rx_qs.filter(is_controlled=True, prescribed_at__date=today).count(),
            },
            "dispensing": {
                "queued": dispense_qs.filter(status="queued").count(),
                "in_progress": dispense_qs.filter(status="in_progress").count(),
                "pending_verification": dispense_qs.filter(status="verification_pending").count(),
            ...
            "interactions": {
                ...
                "severe": interaction_qs.filter(alert_status="active", severity="severe").count(),
                "contraindicated": interaction_qs.filter(alert_status="active", severity="contraindicated").count(),
...
class MedicationSafetyDashboardView(APIView):
    ...
    def get(self, request):
        ...
        qs = MedicationSafetyEvent.objects.filter(
            tenant_id=tenant_id, occurred_at__gte=from_date
        )
