...
class EditionService:
    ...
    @staticmethod
    def get_edition(product_code: str, edition_code: str) -> Optional[ProductEdition]:
        try:
    ...
    def get_edition_features(edition: ProductEdition) -> dict:
        ...
        return {
            ef.feature_code: ef.is_enabled
            for ef in EditionFeature.objects.filter(edition=edition)
        }
    ...
    def get_edition_limits(edition: ProductEdition) -> dict:
        ...
        return {
            el.resource_name: el.max_value
            for el in EditionLimit.objects.filter(edition=edition)
        }
    ...
    def get_edition_modules(edition: ProductEdition) -> list:
        ...
        return [
            em.module_code
            for em in EditionModule.objects.filter(edition=edition, is_included=True)
        ]
    ...
    @classmethod
    def provision_edition(cls, product_code: str, edition_code: str) -> Optional[ProductEdition]:
