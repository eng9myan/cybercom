...
from products.cymed.clinic.appointments.models import ClinicAppointment, AppointmentWaitlist, AppointmentRule
from products.cymed.clinic.queues.models import Queue, QueueEntry, QueueBoard
from products.cymed.clinic.triage.models import TriageAssessment, TriageVitalSigns, TriageRiskScore
from products.cymed.clinic.consultations.models import Consultation, ConsultationDiagnosis
from products.cymed.clinic.specialties.models import SpecialtyProfile, SpecialtyTemplate
from products.cymed.clinic.clinical_forms.models import ClinicalForm, ClinicalFormSection, ClinicalFormField, ClinicalFormSubmission
from products.cymed.clinic.telemedicine.models import VirtualVisit, VirtualConsent
from products.cymed.clinic.referrals.models import ReferralReason, ReferralProvider, Referral
from products.cymed.clinic.insurance_bridge.models import Payer, InsurancePlan, EligibilityCheck, AuthorizationRequest
...
class TestClinicEdition:
    def test_reception_checkin_checkout_queue(self, auth_client, test_tenant_id):
        ...
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Samer", last_name="Mona", dob="1992-04-12", mrn="MRN-CLN-01"
        )
        arr_method = ArrivalMethod.objects.create(tenant_id=test_tenant_id, name="Walk-In", code="walkin")
        reason = VisitReason.objects.create(tenant_id=test_tenant_id, name="Fever", code="fever")
        status_model = VisitStatus.objects.create(tenant_id=test_tenant_id, name="Checked In", code="checked_in")
        ...
        assert OutboxEvent.objects.filter(tenant_id=test_tenant_id, event_type="cymed.clinic.checkin.created").count() == 1
    ...
    def test_appointments_double_booking_and_waitlist(self, auth_client, test_tenant_id):
        ...
        rule = AppointmentRule.objects.create(
            tenant_id=test_tenant_id, name="Cardiology Rule", specialty_code="cardiology", allow_conflicting_bookings=False
        )
    ...
    def test_queues_and_waiting_room(self, auth_client, test_tenant_id):
        ...
        queue = Queue.objects.create(
            tenant_id=test_tenant_id, name="Pediatrics Queue", department_id=uuid.uuid4(), code="PED-Q"
        )
        board = QueueBoard.objects.create(
            tenant_id=test_tenant_id, queue=queue, board_name="Pediatrics Board", layout_settings={"theme": "dark"}
        )
    ...
    def test_triage_vital_signs_and_mews_score(self, auth_client, test_tenant_id):
        # 1. Setup CheckIn
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Kareem", last_name="Adel", dob="1980-01-01", mrn="MRN-T-01"
        )
        arr = ArrivalMethod.objects.create(tenant_id=test_tenant_id, name="Emergency", code="er")
        reason = VisitReason.objects.create(tenant_id=test_tenant_id, name="Chest Pain", code="chest_pain")
        status_model = VisitStatus.objects.create(tenant_id=test_tenant_id, name="Arrived", code="arrived")
        checkin = CheckIn.objects.create(
            tenant_id=test_tenant_id, patient=patient, arrival_method=arr, visit_reason=reason, status=status_model
        )
    ...
    def test_consultation_diagnosis_terminology_check(self, auth_client, test_tenant_id):
        # 1. Setup Encounter
        org = Organization.objects.create(tenant_id=test_tenant_id, name="Clinic Org", slug="cl-org", organization_type="clinic")
        fac = Facility.objects.create(tenant_id=test_tenant_id, organization=org, name="Clinic Fac", code="CL-FAC")
        patient = Patient.objects.create(tenant_id=test_tenant_id, first_name="Omar", last_name="Hani", dob="1988-08-08", mrn="MRN-C-01")
        enc = Encounter.objects.create(tenant_id=test_tenant_id, patient=patient, encounter_type="outpatient", status="in_progress", organization=org, facility=fac)
        ...
        response = auth_client.post(
            "/api/v1/clinic/consultations/notes/",
            {
                ...
                "treatment_plan": {"instructions": "Ice compress", "prescriptions": [{"drug": "Panadol", "dosage": "500mg"}]}
            },
            format="json"
        )
        assert response.status_code == 201
        ...
        assert OutboxEvent.objects.filter(tenant_id=test_tenant_id, event_type="cymed.clinic.consultation.created").count() == 1
        ...
        response_invalid = auth_client.post(
            "/api/v1/clinic/consultations/notes/",
            {
                "encounter": enc.id,
                "consulted_by": "Dr. Tareq",
                "diagnoses": [{"code": "INVALID-CODE", "system": "icd11", "display": "Fake disease"}],
            },
            format="json"
        )
    ...
    def test_specialty_profiles_and_clinical_rules(self, auth_client, test_tenant_id):
        # Create Specialty
        spec = SpecialtyProfile.objects.create(tenant_id=test_tenant_id, name="Cardiology", code="cardiology")
        template = SpecialtyTemplate.objects.create(
            tenant_id=test_tenant_id, specialty=spec, name="ECHO report", code="echo", schema_definition={"fields": ["ef_pct"]}
        )
    ...
    def test_clinical_forms_builder_and_submissions(self, auth_client, test_tenant_id):
        ...
        form = ClinicalForm.objects.create(tenant_id=test_tenant_id, name="Consent Form", code="consent")
        sect = ClinicalFormSection.objects.create(tenant_id=test_tenant_id, form=form, title="Section 1", display_order=1)
        field = ClinicalFormField.objects.create(
            tenant_id=test_tenant_id, section=sect, label="agree", field_type="boolean", required=True
        )
    ...
    def test_telemedicine_meeting_session_events(self, auth_client, test_tenant_id):
        # 1. Setup Patient
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Rana", last_name="Fadi", dob="1995-10-10", mrn="MRN-T-02"
        )
        ...
        assert response.data["session"]["connection_url"].startswith("https://cyconnect.cymed.io/meeting/")

        # 3. Trigger starting visit (should emit event)
        visit_id = response.data["id"]
        response_start = auth_client.post(f"/api/v1/clinic/telemedicine/visits/{visit_id}/start_session/")
        assert response_start.status_code == 200
        assert response_start.data["status"] == "in_progress"
        assert OutboxEvent.objects.filter(tenant_id=test_tenant_id, event_type="cymed.clinic.telemedicine.started").count() == 1
    ...
    def test_referrals_and_attachments(self, auth_client, test_tenant_id):
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Tariq", last_name="Omar", dob="2005-02-02", mrn="MRN-R-01"
        )
        reason = ReferralReason.objects.create(tenant_id=test_tenant_id, name="MRI", code="mri")
        prov = ReferralProvider.objects.create(
            tenant_id=test_tenant_id, name="Dr. Ziad", code="ziad", organization_name="Specialized Radiology Center"
        )
        ...
        assert response.status_code == 201
        assert OutboxEvent.objects.filter(tenant_id=test_tenant_id, event_type="cymed.clinic.referral.created").count() == 1

    def test_insurance_bridge_eligibility_and_authorization(self, auth_client, test_tenant_id):
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Jana", last_name="Khaled", dob="1999-09-19", mrn="MRN-I-01"
        )
        payer = Payer.objects.create(tenant_id=test_tenant_id, name="MedNet UAE", payer_code="mednet")
        plan = InsurancePlan.objects.create(
            tenant_id=test_tenant_id, payer=payer, plan_name="Gold Plan", plan_code="gold", copay_percentage=10.0
        )
    ...
    def test_billing_bridge_erp_ledger_posting(self, auth_client, test_tenant_id):
        # 1. Setup Base Data
        org = Organization.objects.create(tenant_id=test_tenant_id, name="Clinic Org", slug="cl-org-b", organization_type="clinic")
        fac = Facility.objects.create(tenant_id=test_tenant_id, organization=org, name="Clinic Fac", code="CL-FAC-B")
        patient = Patient.objects.create(
            tenant_id=test_tenant_id, first_name="Lujain", last_name="Salem", dob="2002-05-15", mrn="MRN-B-01"
        )
        enc = Encounter.objects.create(
            tenant_id=test_tenant_id, patient=patient, encounter_type="outpatient", status="finished", organization=org, facility=fac
        )
        
        code = ChargeCode.objects.create(tenant_id=test_tenant_id, code="99213", display="Outpatient Consultation", category="consultation")
        pr = PriceList.objects.create(tenant_id=test_tenant_id, name="UAE Price List", code="uae-prices", currency="AED")
        service = ClinicService.objects.create(tenant_id=test_tenant_id, price_list=pr, charge_code=code, price=250.00)
