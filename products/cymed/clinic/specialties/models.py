...
class SpecialtyTemplate(BaseModel):
    specialty = models.ForeignKey(
        SpecialtyProfile, on_delete=models.CASCADE, related_name="templates"
    )
    name = models.CharField(max_length=255)
...
class SpecialtyQuestionnaire(BaseModel):
    specialty = models.ForeignKey(
        SpecialtyProfile, on_delete=models.CASCADE, related_name="questionnaires"
    )
...
class SpecialtyClinicalRule(BaseModel):
    specialty = models.ForeignKey(
        SpecialtyProfile, on_delete=models.CASCADE, related_name="clinical_rules"
    )
