    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div>
        ...
        <Panel>
          <table className="w-full">
            <thead><tr><th className={th}>Period</th><th className={th}>Status</th><th className={th}>Payslips</th><th className={`${th} text-right`}>Gross</th><th className={`${th} text-right`}>Net</th><th className={th}></th></tr></thead>
            <tbody>
              {batches.map((b) => (
                <tr key={b.id}>
                  ...
                  <td className={`${td} text-right tabular-nums`}>{money(b.total_gross)}</td>
                  <td className={`${td} text-right tabular-nums`}>{money(b.total_net)}</td>
      ...
      {sel && (
        <div>
          ...
          <Panel>
            <table className="w-full">
              <thead><tr><th className={th}>Employee</th><th className={`${th} text-right`}>Gross</th><th className={`${th} text-right`}>Deductions</th><th className={`${th} text-right`}>Net</th></tr></thead>
              <tbody>
                {slipsFor(sel).map((s) => (
                  <tr key={s.id}><td className={td}>{s.employee_name || s.employee}</td>
                    <td className={`${td} text-right tabular-nums`}>{money(s.gross_pay || s.gross)}</td>
                    <td className={`${td} text-right tabular-nums`}>{money(s.total_deductions || s.deductions)}</td>
                    <td className={`${td} text-right tabular-nums font-semibold`}>{money(s.net_pay || s.net)}</td></tr>
                ))}
