...
present (so the frontend's fake dev JWT flows through), otherwise it injects a
default dev identity. Either way it populates request.user_session exactly like
the real middleware, so downstream tenant isolation and viewsets behave the same.

Hard safety gate: refuses to activate unless DEBUG and CYED_DEV_AUTH=1.
"""
...
from django.http import JsonResponse  # noqa: F401  (parity with cycom shim)
...
class DevAuthMiddleware:
    def __init__(self, get_response):
        if not (settings.DEBUG and os.environ.get("CYED_DEV_AUTH") == "1"):
            raise RuntimeError(
                "DevAuthMiddleware refuses to load without DEBUG=True and CYED_DEV_AUTH=1. "
    ...
    def __call__(self, request):
        ...
        request.user_session = {
            ...
            "email": claims.get("email", "admin@cyed.dev"),
            "tenant_id": claims.get("tenant_id", self.dev_tenant),
            "roles": claims.get("roles")
            or claims.get("realm_access", {}).get("roles", ["tenant_admin"]),
            "permissions": claims.get("permissions", []),
            # Mirrors the real middleware — campus scoping must behave the same
            # in dev, or a bug in it is invisible until production.
            "campus_ids": claims.get("campus_ids") or [],
        }
        return self.get_response(request)
