...
class TestIdentityProvider:
    def test_unique_alias_per_realm(self, active_realm):
        IdentityProvider.objects.create(realm=active_realm, alias="corporate", display_name="Corp", protocol="oidc")
        from django.db import IntegrityError
        with pytest.raises(IntegrityError):
            IdentityProvider.objects.create(realm=active_realm, alias="corporate", display_name="Dup", protocol="oidc")

    def test_different_aliases_ok(self, active_realm):
        IdentityProvider.objects.create(realm=active_realm, alias="a", display_name="A", protocol="oidc")
        IdentityProvider.objects.create(realm=active_realm, alias="b", display_name="B", protocol="saml")
...
class TestServicePrincipal:
    def test_create(self, active_realm):
        sp = ServicePrincipal.objects.create(name="etl", realm=active_realm, client_id="etl-svc", scopes=["read"])
        assert sp.is_active is True
        assert "etl" in str(sp)

    def test_inactive(self, active_realm):
        sp = ServicePrincipal.objects.create(name="x", realm=active_realm, client_id="x", is_active=False)
...
class TestApplicationClient:
    def test_create_oidc(self, active_realm):
        c = ApplicationClient.objects.create(
            realm=active_realm, client_id="web", name="Web", protocol=ClientProtocol.OIDC,
            public_client=True, redirect_uris=["http://app/cb"],
        )
...
class TestClientSecret:
    ...
    def test_revoke(self, active_realm):
        c = ApplicationClient.objects.create(realm=active_realm, client_id="x", name="X")
        cs = ClientSecret.objects.create(tenant_id=active_realm.tenant_id, client=c, secret_hash="h", secret_hint="abcd")
        cs.revoke()
        cs.refresh_from_db()
...
class TestRoleAndPermission:
    ...
    def test_role_assignment_permission_only(self, active_realm):
        ...
        ra = RoleAssignment.objects.create(
            role=role, permission=perm, kind="permission",
        )
...
class TestGroupAndMembership:
    def test_group_hierarchy(self, active_realm):
        ...
        child = Group.objects.create(realm=active_realm, name="leaf", path="/root/leaf", parent=parent)
        assert child.parent_id == parent.id
        assert child in parent.children.all()

    def test_group_membership_unique(self, active_realm):
        g = Group.objects.create(realm=active_realm, name="g", path="/g")
        GroupMembership.objects.create(tenant_id=active_realm.tenant_id, group=g, user_id=uuid.uuid4())
        from django.db import IntegrityError
        with pytest.raises(IntegrityError):
            GroupMembership.objects.create(tenant_id=active_realm.tenant_id, group=g, user_id=g.memberships.first().user_id)
...
class TestUserProfile:
    def test_record_login_resets_failures(self, active_realm):
        u = UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="alice", email="a@x.io",
    ...
    def test_record_failed_login_locks_after_threshold(self, active_realm):
        u = UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="bob", email="b@x.io",
        )
        for _ in range(5):
    ...
    def test_is_locked_false_when_expired(self, active_realm):
        u = UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="c", email="c@x.io",
    ...
    def test_unique_username_per_realm(self, active_realm):
        UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="dup", email="a@x.io",
        )
        from django.db import IntegrityError
        with pytest.raises(IntegrityError):
            UserProfile.objects.create(
                tenant_id=active_realm.tenant_id, realm=active_realm,
                keycloak_user_id=uuid.uuid4(), username="dup", email="b@x.io",
            )
...
class TestUserSession:
    def _session(self, active_realm):
        u = UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="u", email="u@x.io",
        )
...
class TestLoginAudit:
    def test_create(self, active_realm):
        e = LoginAudit.objects.create(
            tenant_id=active_realm.tenant_id,
            realm=active_realm, outcome="success",
            username_attempted="u", ip_address="10.0.0.1",
        )
...
class TestDeviceAndWebAuthn:
    def test_device_active(self, active_realm):
        ...
        d = DeviceRegistration.objects.create(
            tenant_id=active_realm.tenant_id, user=u, name="iPhone",
            device_type="mobile", platform="iOS",
        )
    ...
    def test_device_revoked(self, active_realm):
        ...
        d = DeviceRegistration.objects.create(
            tenant_id=active_realm.tenant_id, user=u, name="k", device_type="hw_key",
    ...
    def test_webauthn_unique_credential_id(self, active_realm):
        ...
        WebAuthnCredential.objects.create(
            tenant_id=active_realm.tenant_id, user=u, credential_id="abc",
            public_key="k", label="k1",
        )
        from django.db import IntegrityError
        with pytest.raises(IntegrityError):
            WebAuthnCredential.objects.create(
                tenant_id=active_realm.tenant_id, user=u, credential_id="abc",
                public_key="k2", label="k2",
            )
...
class TestBreakGlass:
    def _user(self, active_realm):
        return UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="doc", email="d@x.io",
        )

    def test_lifecycle(self, active_realm):
        user = self._user(active_realm)
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.CLINICAL, justification="Mass casualty incident",
    ...
    def test_approve_requires_dual(self, active_realm):
        ...
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.SECURITY, justification="Active intrusion",
            target_resource="audit", target_action="disable-waf",
        )
    ...
    def test_activate_requires_approved(self, active_realm):
        ...
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.OPERATIONAL, justification="Outage",
            target_resource="db", target_action="failover",
        )
        with pytest.raises(ValueError):
    ...
    def test_expire_due(self, active_realm):
        ...
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.COMPLIANCE, justification="Legal hold",
            target_resource="ledger", target_action="freeze-account",
        )
    ...
    def test_revoke(self, active_realm):
        ...
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.CLINICAL, justification="",
            target_resource="x", target_action="y",
        )
        bg.revoke()
        bg.refresh_from_db()
...
class TestKeycloakAdminClient:
    def test_authenticate_returns_token(self, fake_kc):
        with patch.object(settings := __import__("django").conf.settings, "KEYCLOAK_ENABLED", False, create=True):
    ...
    def test_client_secret_round_trip(self, fake_kc):
        with patch.object(__import__("django").conf.settings, "KEYCLOAK_ENABLED", False, create=True):
            fake_kc.authenticate()
            c = fake_kc.create_client("r1", {"clientId": "c1"})
    ...
    def test_register_creates_client(self, active_realm):
        with patch.object(__import__("django").conf.settings, "KEYCLOAK_ENABLED", False, create=True):
            c = ClientService().register(
                active_realm, client_id="web-app", name="Web App",
                redirect_uris=["http://app/cb"], public_client=True,
            )
...
class TestUserProvisioningService:
    def test_provision_user_creates_profile(self, active_realm):
        with patch.object(__import__("django").conf.settings, "KEYCLOAK_ENABLED", False, create=True):
            u = UserProvisioningService().provision_user(
                active_realm, username="doc1", email="d1@x.io",
    ...
    def test_provision_user_idempotent(self, active_realm):
        with patch.object(__import__("django").conf.settings, "KEYCLOAK_ENABLED", False, create=True):
            u1 = UserProvisioningService().provision_user(active_realm, username="x", email="x@x.io")
            u2 = UserProvisioningService().provision_user(active_realm, username="x", email="x@x.io")
        assert u1.id == u2.id
...
class TestSessionService:
    def _session(self, active_realm, user=None):
        u = user or UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username=f"u-{uuid.uuid4().hex[:8]}", email=f"u-{uuid.uuid4().hex[:8]}@x.io",
        )
        return UserSession.objects.create(
            tenant_id=active_realm.tenant_id, user=u,
            keycloak_session_id=str(uuid.uuid4()),
        )
    ...
    def test_revoke_all_for_user(self, active_realm):
        user = UserProfile.objects.create(
            tenant_id=active_realm.tenant_id, realm=active_realm,
            keycloak_user_id=uuid.uuid4(), username="alice-revoke", email="alice@x.io",
        )
...
class TestAuditService:
    def test_record_login_increments_metrics(self, active_realm):
        ...
        AuditService.record_login(
            realm=active_realm, username_attempted="u", outcome="success",
            ip_address="10.0.0.1",
        )
    ...
    def test_record_login_failure(self, active_realm):
        ...
        AuditService.record_login(
            realm=active_realm, username_attempted="u", outcome="failure",
    ...
    def test_record_emits_to_platform_sink(self, active_realm):
        ...
        from platform.cyidentity.services import AuditService
        AuditService.record(
            action="create", resource_type="realm",
            resource_id=str(active_realm.id), tenant_id=active_realm.tenant_id,
        )
...
class TestTokenValidateAPI:
    def test_invalid_token(self, admin_client):
        resp = admin_client.post("/api/v1/identity/token/validate/", {"token": "garbage"}, format="json")
...
class TestCyIdentityTasks:
    def test_expire_break_glass_task(self, active_realm):
        ...
        bg = BreakGlassService().request(
            user=user, realm=active_realm,
            reason=BreakGlassReason.CLINICAL, justification="x" * 20,
            target_resource="x", target_action="y",
        )
        BreakGlassService().approve(bg, approver="a", second_approver="b")
        BreakGlassService().activate(bg, duration_seconds=1)
