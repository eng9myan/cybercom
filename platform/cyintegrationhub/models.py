...
class IntegrationPartner(PlatformModel):
    ...
    direction = models.CharField(
        max_length=20, default="bidirectional"
...
class ConnectorConfig(PlatformModel):
    ...
    partner = models.ForeignKey(
        IntegrationPartner, on_delete=models.CASCADE, related_name="connectors"
    )
    name = models.CharField(max_length=255)
    connector_type = models.CharField(
        max_length=50
    )  # fhir, hl7v2, dicom, ldap, keycloak, soap, rest
...
class MessageAuditLog(models.Model):
    ...
    partner = models.ForeignKey(
        IntegrationPartner, on_delete=models.SET_NULL, null=True, blank=True
    )
