#!/usr/bin/env bash
# ============================================================
#  CyVault API — Production Deploy
#  Run on the OCI VM by .github/workflows/deploy-cyvault.yml.
#
#  Docker Compose based, mirroring cycom's/cymed's proven pattern (same
#  VM, same deploy discipline). Deploys to its own isolated stack at
#  /opt/cyvault-api (port 8013, own Postgres/MinIO) - reuses the shared
#  Keycloak already running as part of cymed's stack via an external
#  Docker network reference, rather than duplicating it.
#
#  Release layout (see deploy-cyvault.yml's bundle step):
#    release_dir/cyvault/         — Django project (manage.py etc.)
#    release_dir/platform/        — shared package, sibling of cyvault/
#    release_dir/shared/          — shared package, sibling of cyvault/
#    release_dir/infrastructure/  — Dockerfile.cyvault
#    release_dir/docker-compose.cyvault-api.yml
# ============================================================
set -Eeuo pipefail

release="${1:?usage: deploy-production.sh <git-sha>}"
app_root="${APP_ROOT:-/opt/cyvault-api}"
release_dir="${RELEASE_DIR:-$app_root/releases/$release}"
shared_env="$app_root/shared/.env.production"
compose="docker compose --env-file $shared_env -f docker-compose.cyvault-api.yml"
port="${APP_PORT:-8013}"
...
# directive in the compose YAML resolves relative to the compose file's
# own directory, separately from the --env-file CLI flag above.
...
$compose up -d cyvault-postgres cyvault-minio
echo "    Waiting for postgres to be ready..."
pg_ready=0
for attempt in $(seq 1 60); do
  if $compose exec -T cyvault-postgres sh -c 'pg_isready -U "$POSTGRES_USER"' >/dev/null 2>&1; then
...
if [[ "$pg_ready" != 1 ]]; then
  ...
  $compose logs --tail=100 cyvault-postgres >&2 || true
  exit 5
fi
