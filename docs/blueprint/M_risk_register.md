...
| R03 | **Tenant isolation defect** — cross-tenant data exposure. Kills trust, regulatory breach. Cross-tenant read leak already found once (fixed in `D:\cybercom`). | 3 | 5 | 15 | Defence-in-depth (queryset + RLS + per-tenant DEK + object prefix); automated cross-tenant access test suite fails-closed in CI every build; pentest before each GA. Any confirmed exposure → halt onboarding. | Backend + Sec | Mitigating |
...
| R05 | **CyMed runtime unverified** — ~68k LOC, never run this cycle. HealthFlavour timeline assumes it's real. | 4 | 4 | 16→6 | **VERIFIED 2026-09-04** (`verification/cymed-runtime-2026-09-04.md`): `manage.py check` clean, migrations clean, **pytest 486 pass / 23 fail / 6 err (~95%)**. It is real code, not scaffold. Residual = a ~1-week Phase-1 test-hardening task (payments `tenant_id` gap is a real bug feeding R03; rcm/nphies/hospital failures are fixture/isolation issues). Fixed `requests` missing from `requirements.txt`. | Health pod | Mitigating |
| R06 | **PHI / health-data incident** — breach, mis-disclosure, residency violation. Regulatory + reputational catastrophe. | 2 | 5 | 10→ treat as High | Field encryption, access audit 100%, consent enforcement, in-Kingdom storage, DPIA per flavor, PHI-path pentest before Health GA, zero-incident gate. | Sec + Health | Open |
