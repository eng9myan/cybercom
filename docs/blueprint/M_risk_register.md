...
| R03 | **Tenant isolation defect** — cross-tenant data exposure. Kills trust, regulatory breach. | 3 | 5 | 15→10 | Defence-in-depth: **write side now covered** — `TenantScopedMixin.save()` fills `tenant_id` from ambient context or raises `TenantContextMissing` (2026-09-04), closing the "forgot tenant_id" bug class found in the CyMed audit. Still to do: Postgres RLS rollout + per-tenant DEK + object prefix + automated cross-tenant access suite in CI + pentest before each GA. Any confirmed exposure → halt onboarding. | Backend + Sec | Mitigating |
