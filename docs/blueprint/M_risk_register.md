...
| R06 | **PHI / health-data incident** — breach, mis-disclosure, residency violation. | 2 | 5 | 10→7 | **Field-encryption infra shipped** (`EncryptedText`, per-tenant AES-256-GCM DEK, blind-index lookup, PII registry + `dump_pii_map`, 2026-09-04). Still: convert the actual PHI/PII columns (per-model migrations), access audit 100%, consent enforcement, in-region storage, DPIA per flavor, PHI-path pentest before Health GA, zero-incident gate. | Sec + Health | Mitigating |
