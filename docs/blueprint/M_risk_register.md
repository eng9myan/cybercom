...
| R06 | **PHI / health-data incident** — breach, mis-disclosure, residency violation. | 2 | 5 | 10→4 | **Field-encryption infra + broad column rollout shipped** (`EncryptedText` + `EncryptedJSON`, per-tenant AES-256-GCM DEK, blind-index lookup, PII registry + `dump_pii_map`, 2026-09-04). **111 encrypted columns** (104 CyMed / 7 CyCom), incl. 3 JSON (prescriptions / anesthetic agents / structured findings): patient identifiers + contact/address; the full clinical-documentation surface (documents, encounters, consultations, triage); acute care (inpatient, nursing, ICU, anesthesia, ED, ADT, discharge, maternity, OR consent); `core/clinical` FHIR resources; laboratory results/orders; radiology reports; Hakeem audit national IDs; CyCom `Partner`/`Employee` contact + IBAN. Still: lower-priority operational/QA notes, `EncryptedJSON` for prescription lists, access audit 100%, consent enforcement, in-region storage, DPIA per flavor, PHI-path pentest before Health GA, zero-incident gate. | Sec + Health | Mitigating |
...
| R16 | **Financial correctness bug in prod** — GL imbalance, tax miscalc, or a lost-update double-posting the same GL entry under concurrent requests. | 2 | 5 | 10→7 | Property-based tests (GL always balances, tax = Σ line tax); immutable posted entries; freeze finance deploys on any prod imbalance. **2026-09-04 (M8):** `save_if_unchanged` row_version CAS + `transaction.atomic()` on invoice posting, payment posting, and payroll-run posting — a concurrent post of the same record rolls back its GL entry instead of double-posting; same pattern on stock-move apply. | Finance pod | Mitigating |
